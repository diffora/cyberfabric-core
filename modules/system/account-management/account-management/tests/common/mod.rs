...
/// `acquire` observes the gate as already held. Mirrors the donor's
/// `pre_populate_gate`. Returns the synthetic `worker_id` so the
/// caller can DELETE the row after the assertion.
pub async fn pre_populate_gate(provider: &Arc<AmDbProvider>, scope_key: &str) -> Result<Uuid> {
...
/// Release the synthetic gate row — the next operation on the same
/// `scope_key` MUST succeed (gate is non-sticky).
///
/// Filters on **both** `scope_key` and `worker_id`, mirroring the
/// production `lock::release` contract. Filtering by `worker_id`
/// alone would let one helper invocation drop multiple synthetic
/// gates for the same worker and make the contention tests less
/// representative than the real lock surface.
pub async fn release_gate(
    ...
    integrity_check_runs::Entity::delete_many()
        .filter(
            Condition::all()
                .add(integrity_check_runs::Column::ScopeKey.eq(scope_key))
                .add(integrity_check_runs::Column::WorkerId.eq(worker_id)),
        )
        .secure()
        .scope_with(&allow)
        .exec(&conn)
