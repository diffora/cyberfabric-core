...
async fn repair_inserts_missing_self_row() {
    ...
    let report = h
        .repo
        .repair_derivable_closure_violations(&allow_all(), IntegrityScope::Whole)
        .await
        .expect("repair");
...
async fn repair_concurrent_burst_observes_only_gate_or_success() {
    ...
    for _ in 0..TASK_COUNT {
        ...
        handles.push(tokio::spawn(async move {
            barrier.wait().await;
            repo.repair_derivable_closure_violations(
    ...
    for handle in handles {
        match handle.await.expect("task panicked") {
            Ok(_) => ok_count += 1,
            Err(DomainError::IntegrityCheckInProgress { ref scope }) if scope == "whole" => {
                gate_count += 1;
            }
