...
impl<R: TenantRepo> BootstrapService<R> {
    ...
    async fn step_initial_preflight_and_wait(&self, ctx: &mut RunCtx) -> BootstrapState {
        ...
        // config would burn the full `idp_wait_timeout_secs` budget
    ...
    /// burning the full `idp_wait_timeout_secs` budget.
    ///
    ...
    async fn preflight_root_gts_config(&self, deadline: Instant) -> Result<(), DomainError> {
        ...
        // Opt-in plugin-advertised input-schema validation for the
        // operator-supplied `root_tenant_metadata`. Plugins that
        // override `provision_input_schema_id` get the configured
        // metadata pre-checked at bootstrap so a misconfigured TOML
        // surfaces as a `Validation` error here rather than as a
        // downstream plugin failure after the `provisioning` row is
        // already inserted. Plugins that inherit the default (`None`)
        // keep the opaque pass-through behaviour.
