...
impl<R: TenantRepo> BootstrapService<R> {
    ...
    ///
    /// Module wiring (`module.rs::init`) is the canonical
    /// [`BootstrapConfig::validate`] call site because it owns the
    /// strict/non-strict branching: a strict-mode validation failure is
    /// lifecycle-fatal, whereas a non-strict failure logs and skips
    /// bootstrap entirely. The `debug_assert!` here is a defence-in-depth
    /// trip-wire for **other** call sites (tests, future embedders) that
    /// may construct the saga directly with a default-defaulted config —
    /// `BootstrapConfig` uses `serde(default)`, so an empty TOML table
    /// deserialises to `Uuid::nil()` and silently breaks the
    /// `fr-bootstrap-idempotency` contract on the next platform restart
    /// (see `feature-platform-bootstrap.md` lines 23-25).
    #[must_use]
    pub fn new(
        repo: Arc<R>,
        idp: Arc<dyn IdpTenantProvisioner + Send + Sync>,
    ...
    pub async fn run(&self) -> Result<TenantModel, AmError> {
        // Eager type-registry preflight: a missing or malformed GTS
        // tenant-type is a permanent misconfiguration, not a transient
        // outage, so we want to fail fast — before sinking minutes into
        // the IdP-availability probe. The check only matters on the
        // fresh-insert path; classify is one cheap `find_by_id` call,
        // and the safety-net preflight inside `try_bootstrap_once_locked`
        // remains for other entry paths.
        let probe_scope = AccessScope::allow_all();
        if matches!(
            self.classify(&probe_scope).await?,
    ...
    //
    // Metric phase label is `idp_precheck` (not `idp_waiting`) so that
    // dashboards can isolate the bounded pre-saga availability probe
    // from the open-ended saga retry loop in `run()`. Both surfaces
    // emit on `AM_BOOTSTRAP_LIFECYCLE` but with distinct phase labels.
...
mod tests {
    ...
    fn tenant_type_entity(allowed_parent_types: &[&str]) -> GtsEntity {
        GtsEntity {
    ...
    impl TypesRegistryClient for FakeTypesRegistry {
        ...
        async fn get(&self, _gts_id: &str) -> Result<GtsEntity, TypesRegistryError> {
            match &*self.outcome.lock().expect("lock") {
                RegistryOutcome::RootEligible => Ok(tenant_type_entity(&[])),
                RegistryOutcome::NonRoot => Ok(tenant_type_entity(&[
