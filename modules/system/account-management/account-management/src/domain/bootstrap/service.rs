...
impl<R: TenantRepo> BootstrapService<R> {
    ...
    pub async fn run(&self) -> Result<TenantModel, DomainError> {
        ...
        let mut ctx = RunCtx {
            ...
            deadline: Instant::now() + Duration::from_secs(self.cfg.idp_wait_timeout_secs),
            ...
            // peer's saga budget is bounded by
            // `idp_wait_timeout_secs`, so anything older than 2× is
            // by definition not in flight).
            stuck_threshold: time::Duration::seconds(
                i64::try_from(self.cfg.idp_wait_timeout_secs.saturating_mul(2))
                    .unwrap_or(i64::MAX),
            ),
    ...
    async fn step(&self, state: BootstrapState, ctx: &mut RunCtx) -> BootstrapState {
        match state {
            ...
            BootstrapState::Terminal(_) => unreachable!(
                "Terminal must short-circuit `run()` before re-entering `step`",
            ),
        }
    }
    ...
    async fn classify_with_terminal_dispatch(&self, ctx: &RunCtx) -> ClassifyOutcome {
        ...
        match cls {
            ...
            BootstrapClassification::InvariantViolation { observed_status } => ClassifyOutcome::Terminal(
                BootstrapState::Terminal(Err(handle_invariant_violation(observed_status))),
            ),
    ...
    async fn step_initial_preflight_and_wait(&self, ctx: &mut RunCtx) -> BootstrapState {
        ...
        if let Err(e) = self.preflight_root_tenant_type().await {
        ...
        match self.insert_root_provisioning(&ctx.scope).await {
            ...
            Err(DomainError::AlreadyExists { .. }) => {
                ...
                if Instant::now() >= ctx.deadline {
                    ...
                    return BootstrapState::Terminal(Err(DomainError::IdpUnavailable {
                        detail:
                            "bootstrap deadline exhausted while losing concurrent-root inserts"
                                .to_owned(),
                    }));
                }
                BootstrapState::Sleep {
    ...
    async fn step_sleep(&self, ctx: &mut RunCtx, reason: SleepReason) -> BootstrapState {
        ...
        // Surface the call-site reason on a stable trace event so
        // operators can attribute a wake-up cycle without re-
        // correlating per-arm metric counters. The reason maps to
        // a fixed string label (`SleepReason::as_label`) so a
        // derive change on the enum cannot silently rename the
        // value dashboards filter on.
        tracing::trace!(
            target: "am.bootstrap",
