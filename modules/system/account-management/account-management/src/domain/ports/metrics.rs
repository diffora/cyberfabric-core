...
use account_management_sdk::idp::{CheckAvailabilityFailure, DeprovisionFailure, ProvisionFailure};
use account_management_sdk::idp_user::{DeprovisionUserOutcome, UserOperationFailure};
...
/// associated constants, and `From<&ProvisionFailure>` /
/// `From<&CheckAvailabilityFailure>` bridge the SDK failure enums
/// whose `as_metric_label()` helpers already own the variant→string
/// mapping. No public constructor — values must come from a constant
/// or a `From` impl, so the cardinality surface stays closed.
#[domain_model]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl From<&ProvisionFailure> for BootstrapClassification {
    fn from(f: &ProvisionFailure) -> Self {
        Self(f.as_metric_label())
    }
}

impl From<&CheckAvailabilityFailure> for BootstrapClassification {
    fn from(f: &CheckAvailabilityFailure) -> Self {
        Self(f.as_metric_label())
    }
}
...
/// (`ProvisionFailure`, `DeprovisionFailure`, `UserOperationFailure`,
...
impl From<&ProvisionFailure> for DependencyOutcome {
    fn from(f: &ProvisionFailure) -> Self {
        Self(f.as_metric_label())
    }
}

impl From<&DeprovisionFailure> for DependencyOutcome {
    fn from(f: &DeprovisionFailure) -> Self {
        Self(f.as_metric_label())
    }
}

impl From<&UserOperationFailure> for DependencyOutcome {
    fn from(f: &UserOperationFailure) -> Self {
        Self(f.as_metric_label())
    }
}

impl From<DeprovisionUserOutcome> for DependencyOutcome {
    fn from(o: DeprovisionUserOutcome) -> Self {
