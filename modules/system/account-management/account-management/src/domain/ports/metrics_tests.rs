...
fn newtype_from_failure_bridges_use_as_metric_label() {
    ...
    let provision_fail = ProvisionFailure::Ambiguous {
        detail: String::new(),
    };
    assert_eq!(
    ...
    let deprov_fail = DeprovisionFailure::NotFound {
        detail: String::new(),
    };
    assert_eq!(
    ...
    let user_fail = UserOperationFailure::Unavailable {
        detail: String::new(),
    };
    assert_eq!(
    ...
    let user_outcome = DeprovisionUserOutcome::Removed;
    assert_eq!(
        DependencyOutcome::from(user_outcome).as_str(),
        user_outcome.as_metric_label(),
    );

    let check_fail = CheckAvailabilityFailure::Unreachable {
        detail: String::new(),
    };
    assert_eq!(
        BootstrapClassification::from(&check_fail).as_str(),
        check_fail.as_metric_label(),
    );
