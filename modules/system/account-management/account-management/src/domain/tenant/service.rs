...
use crate::domain::metrics::{
    ...
    AM_TENANT_RETENTION, MetricKind, emit_gauge_value, emit_metric,
};
...
use crate::domain::tenant::integrity::{
    IntegrityCategory, IntegrityReport, IntegrityScope, Violation,
};
...
impl<R: TenantRepo> TenantService<R> {
    ...
    /// `audit_integrity_for_scope` call.
    ...
    pub async fn check_hierarchy_integrity(
        ...
        // SQL-side classification + single-flight gating happen inside
        // the repo call. Concurrent contention surfaces as
        // `AmError::AuditAlreadyRunning` (HTTP 429) and is propagated
        // here unchanged via `?`. Whole-tree memory is `O(violations)`,
        // so no in-memory cap is required at the service layer.
        let pairs = self
            .repo
            .audit_integrity_for_scope(&AccessScope::allow_all(), scope.clone())
            .await?;

        // Bucket the (category, violation) pairs into a per-category
        // map. The classifier may return categories in any order; the
        // report shape requires exactly one entry per
        // `IntegrityCategory::all()` variant in fixed order, including
        // zero-valued entries.
        ...
        // Emit one gauge sample per category — always, including
        // zero-valued — so the dashboard can distinguish "no
        // violations" from "checker never ran". Cycle detection
        // additionally emits a counter at the SQL detection boundary
        // (existing pattern in `integrity::record_cycle_detected`).
        // `emit_gauge_value` carries the numeric category count into
