...
use account_management_sdk::{
    DeprovisionUserRequest, IdpPluginClient, ListUsersRequest, NewUser, ProvisionUserRequest,
    TenantContext, User, UserPagination,
};
...
/// `display_name`) enforced at the AM boundary as
...
/// values straight to the `IdP`, turning AM into a proxy that the
/// published GTS contract pretends to validate.
///
...
impl UserService {
    ...
    pub async fn provision_user(
        ...
        // these AM-side caps a megabyte-scale `email` or
        // `display_name` would reach the `IdP` verbatim).
        ...
        match outcome {
            Ok(projection) => {
                ...
                // contract: a successful `Ok(User)` is, by
