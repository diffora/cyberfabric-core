...
use account_management_sdk::{
    CheckAvailabilityFailure, DeprovisionUserOutcome, DeprovisionUserRequest, IdpPluginClient,
    ListUsersRequest, ProvisionUserRequest, UserOperationFailure, UserPage, UserProjection,
};
...
///
/// `OkNotFound` is meaningful ONLY for `deprovision_user` (where it maps
/// to `DeprovisionUserOutcome::NotFoundInTenant`, the idempotency guard's
/// absent-equivalent). Setting it on `provision_user` / `list_users`
...
pub enum FakeUserOutcome {
    /// `deprovision_user` returns `Ok(Removed)`; `provision_user` returns the
    /// configured projection; `list_users` returns the configured page.
    Ok,
    /// `deprovision_user` returns `Ok(NotFoundInTenant)` (idempotency
    /// guard branch). MUST NOT be set on `provision_user` / `list_users`
...
impl IdpPluginClient for FakeIdpUserProvisioner {
    async fn check_availability(&self) -> Result<(), CheckAvailabilityFailure> {
        // User-side fake assumes the IdP is reachable; tests that
        // exercise availability failure use `FakeIdpProvisioner`
        // instead.
        Ok(())
    }
        ...
        match oc {
            FakeUserOutcome::Ok => {
                ...
                Ok(projection.unwrap_or_else(|| {
                    let mut p = UserProjection::new(Uuid::new_v4(), req.payload.username.clone());
                    ...
                    if let Some(avatar_url) = req.payload.avatar_url.clone() {
                        p = p.with_avatar_url(avatar_url);
                    }
                    if let Some(attributes) = req.payload.attributes.clone() {
                        p = p.with_attributes(attributes);
                    }
                    p
                }))
            }
            FakeUserOutcome::OkNotFound => panic!(
                "FakeIdpUserProvisioner: FakeUserOutcome::OkNotFound is only meaningful \
                 for deprovision_user; setting it on provision_user is a misconfigured test"
            ),
    ...
    async fn deprovision_user(
        ...
        match oc {
            FakeUserOutcome::Ok => Ok(DeprovisionUserOutcome::Removed),
            FakeUserOutcome::OkNotFound => Ok(DeprovisionUserOutcome::NotFoundInTenant),
            FakeUserOutcome::Unavailable => Err(UserOperationFailure::Unavailable {
                detail: "fake unavailable".into(),
            }),
            FakeUserOutcome::Unsupported => Err(UserOperationFailure::UnsupportedOperation {
    ...
    async fn list_users(&self, req: &ListUsersRequest) -> Result<UserPage, UserOperationFailure> {
        ...
        match oc {
            FakeUserOutcome::OkNotFound => panic!(
                "FakeIdpUserProvisioner: FakeUserOutcome::OkNotFound is only meaningful \
                 for deprovision_user; setting it on list_users is a misconfigured test"
            ),
            FakeUserOutcome::Ok => {
                ...
                // Apply pagination so the fake actually emulates a
                // paginating IdP: previously the entire `filtered`
                // Vec was passed straight to `UserPage::new`, leaving
                // `top` / `skip` as metadata-only inputs that
                // downstream tests could not exercise. The slice
                // matches the production SDK contract — `skip` is an
                // offset, `top` is a max cap on the returned items —
                // so a regression that ignores pagination on either
                // side now surfaces in the fake-backed unit tests.
                let skip = usize::try_from(req.pagination.skip()).unwrap_or(usize::MAX);
                let top = usize::try_from(req.pagination.top()).unwrap_or(usize::MAX);
                let paged: Vec<_> = filtered.into_iter().skip(skip).take(top).collect();
                Ok(UserPage::new(
                    paged,
                    req.pagination.top(),
                    req.pagination.skip(),
