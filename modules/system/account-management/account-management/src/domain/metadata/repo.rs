...
pub trait MetadataRepo: Send + Sync {
    ...
    /// List the tenant's direct entries, paginated. Order is stable on
    /// `(schema_uuid)` so cursor re-reads are deterministic. Inherited
    /// values from ancestors are NOT walked here — listing is
    /// per-tenant only per FEATURE §3.1 (the `/resolved` endpoint owns
    /// inheritance and lives on the service layer).
    ///
    /// Returns a [`MetadataRowsPage`] carrying the page rows plus the
    /// unfiltered `total` count so the service can build the public
    /// list envelope without a second round-trip.
