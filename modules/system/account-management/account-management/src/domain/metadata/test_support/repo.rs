...
impl MetadataRepo for FakeMetadataRepo {
    async fn list_for_tenant(
        ...
        // deterministic. Mirrors the contract documented on the trait.
        all.sort_by_key(|r| r.schema_uuid);

        // Total is the unfiltered count BEFORE slicing — production
        // `MetadataRepoImpl::list_for_tenant` issues a separate
        // `count(*)` query so the public list envelope can report the
        // true match-count independent of `top` / `skip`.
        let total = u64::try_from(all.len()).unwrap_or(u64::MAX);
        let skip = pagination.skip as usize;
        let top = pagination.top as usize;
        let rows: Vec<MetadataRow> = all.into_iter().skip(skip).take(top).collect();
        Ok(MetadataRowsPage { rows, total })
    }
    ...
    async fn upsert_for_tenant(
        ...
        if let Some(existing) = state.rows.get_mut(&key) {
            ...
            // `updated_at`, rewrite the opaque value.
