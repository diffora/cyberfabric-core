...
use crate::domain::metadata::{MetadataPagination, UpsertOutcome};
...
async fn fake_repo_full_lifecycle_round_trip() {
    ...
    let inserted_hi = repo
        .upsert_for_tenant(&scope(), tenant_a, schema_hi, json!({"i": 3}), t0)
        .await
        .expect("insert hi");
    ...
    repo.upsert_for_tenant(&scope(), tenant_a, schema_lo, json!({"i": 1}), t0)
        .await
        .expect("insert lo");

    // Unrelated tenant's row — MUST not surface in tenant_a's list.
    repo.upsert_for_tenant(&scope(), tenant_b, schema_lo, json!({"i": 99}), t0)
        .await
        .expect("insert b/lo");
    ...
    let updated = repo
        .upsert_for_tenant(&scope(), tenant_a, schema_hi, json!({"i": 4}), t1)
        .await
        .expect("update hi");
    ...
    // List returns only tenant_a's rows in `schema_uuid` order.
    let page = repo
        .list_for_tenant(&scope(), tenant_a, MetadataPagination::unlimited())
        .await
        .expect("list");
    let schemas: Vec<Uuid> = page.rows.iter().map(|r| r.schema_uuid).collect();
    assert_eq!(schemas, vec![schema_lo, schema_hi]);
    assert!(page.rows.iter().all(|r| r.tenant_id == tenant_a));
    assert_eq!(
        page.total, 2,
        "total reflects unfiltered tenant_a row count"
    );
