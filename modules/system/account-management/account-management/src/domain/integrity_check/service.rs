...
//!   per-scope single-flight gate; emit
...
use crate::domain::tenant::integrity::{IntegrityReport, IntegrityScope, RepairReport};
...
/// loop. Kept narrow on purpose — the loop only ever needs the
/// whole-tree variants in v1, so the trait surface stays a two-method
/// pair, and the in-process consumer (the loop) does not become a
/// vehicle for accidentally exposing scoped variants the SDK has not
/// yet shipped.
#[async_trait]
...
impl<R: TenantRepo + 'static> IntegrityChecker for TenantService<R> {
    async fn run_whole_integrity_check(&self) -> Result<IntegrityReport, DomainError> {
        self.check_hierarchy_integrity(IntegrityScope::Whole).await
    }

    async fn run_whole_integrity_repair(&self) -> Result<RepairReport, DomainError> {
        self.repair_hierarchy_integrity(IntegrityScope::Whole).await
    }
}
...
async fn trigger_auto_repair(checker: &dyn IntegrityChecker) {
    ...
    match checker.run_whole_integrity_repair().await {
        Ok(report) => {
            ...
            // Emit the repair duration with `scope=whole_repair` so
            // dashboards can disaggregate the auto-repair phase from
            // the preceding check phase (which uses
            // `scope=whole`). Without this an operator looking at
