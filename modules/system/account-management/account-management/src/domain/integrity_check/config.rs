...
pub struct IntegrityRepairConfig {
    ...
    /// auto-trigger from the periodic loop are inert — calls return
    /// an empty [`crate::domain::tenant::integrity::RepairReport`]
...
impl IntegrityCheckConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.interval_secs < Self::MIN_INTERVAL_SECS {
            bad.push(
                "integrity_check.interval_secs (must be >= 60; the per-scope single-flight gate serialises ticks at coarser granularity)",
            );
        }
