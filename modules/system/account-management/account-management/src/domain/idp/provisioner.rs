...
//!
//! Phase 1 defines the single method [`IdpTenantProvisioner::provision_tenant`].
//! The `deprovision_tenant` counterpart is added in the deletion-pipeline
//! phase; adding it now would violate YAGNI and create a stub that the
//! service cannot exercise.
//!
...
impl ProvisionFailure {
    ...
    ///
    /// The service layer is the authoritative mapper for the `Ambiguous`
    /// variant (it yields `AmError::Internal` with a diagnostic string).
    /// This helper covers the two cases where compensation already ran:
    /// `CleanFailure` → `idp_unavailable`, `UnsupportedOperation` →
...
mod tests {
    ...
    #[test]
    fn unsupported_operation_maps_to_sub_code_idp_unsupported_operation() {
    ...
    fn deprovision_failure_maps_to_am_error() {
        ...
        .into_am_error();
        assert_eq!(r.sub_code(), "idp_unavailable");
        ...
        .into_am_error();
        assert_eq!(u.sub_code(), "idp_unsupported_operation");
    }
