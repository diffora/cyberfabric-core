...
/// dispatching), and the approver UUID stamped on the resolved row.
///
...
/// re-load. The single seam is documented on the trait method itself.
#[domain_model]
...
pub trait ConversionRepo: Send + Sync {
    ...
    /// Atomic guarded `UPDATE`: stamp `status = approved`,
    /// `approved_by`, and `resolved_at`. Filter is
    /// `WHERE id = :request_id AND status = 0 AND deleted_at IS NULL`.
    ///
    /// On zero rows updated the implementation MUST re-read the current
    /// row to distinguish [`DomainError::NotFound`] from
    /// [`DomainError::AlreadyResolved`]. The full dual-consent apply
    /// (closure barrier rewrite, `tenants.self_managed` flip,
    /// type re-evaluation) is owned by
    /// [`Self::apply_conversion_approval`] — this single-statement
    /// path is retained for the unit-test seams and for future
    /// callers that only need the pending-row transition.
    ...
    /// recheck, status recheck, type re-evaluation,
    ...
    ///
    /// Order inside the TX is fixed: re-evaluate the
    /// parent / child type pairing FIRST (so a type-not-allowed
    /// rejection rolls back without flipping any tenant flag), then
    /// flip `tenants.self_managed`, then rewrite closure barriers,
    /// and finally stamp the request transition. Any failure aborts
    /// the TX so the pending row, the `tenants.self_managed` value,
    /// and every closure barrier remain unchanged. On success the
    /// returned [`ConversionRequest`] carries the post-transition
    /// snapshot.
    ///
    /// # Errors
    ///
    ...
    ///   `Pending`.
    /// * [`DomainError::Validation`] — the converting tenant is no
    ///   longer `Active` (status precondition under TX).
    /// * [`DomainError::TypeNotAllowed`] — the
    ///   parent / child type pairing fails the GTS
    ///   `allowed_parent_types` check.
    ...
    /// [`Self::transition_pending_to_approved`].
