    ...
    fn rebuild_pending_index(&mut self) {
        ...
        for row in self.rows.values() {
            if matches!(row.status, ConversionStatus::Pending) && row.deleted_at.is_none() {
                self.pending_by_tenant.insert(row.tenant_id, row.id);
            }
        }
    }
}
...
pub struct FakeConversionRepo {
    ...
    /// Optional tenant-type checker used by the apply seam. Tests that
    /// want to drive the type-re-eval rejection branch wire a custom
    /// checker; the inert default admits every pairing.
    tenant_type_checker: Option<Arc<dyn TenantTypeChecker + Send + Sync>>,
}
...
impl FakeConversionRepo {
    ...
    /// Wire a custom [`TenantTypeChecker`] used by the apply seam.
    /// Defaults to admit-everything when unset. Tests that drive the
    /// `TypeNotAllowed` branch pass a checker whose
    /// `check_parent_child` returns
    /// [`DomainError::TypeNotAllowed`].
    #[must_use]
    pub fn with_tenant_type_checker(
        mut self,
        checker: Arc<dyn TenantTypeChecker + Send + Sync>,
    ) -> Self {
        self.tenant_type_checker = Some(checker);
        self
    }
    ...
    /// pending-row transitions (`transition_pending_to_approved` /
...
impl ConversionRepo for FakeConversionRepo {
    ...
    async fn apply_conversion_approval(
        ...
        // Type re-eval MUST run before any flip / barrier rewrite.
        if let Some(checker) = self.tenant_type_checker.as_ref() {
            ...
            checker
                .check_parent_child(parent.tenant_type_uuid, tenant.tenant_type_uuid)
                .await?;
        }
