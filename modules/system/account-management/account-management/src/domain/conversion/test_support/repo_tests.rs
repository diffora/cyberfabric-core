...
#[tokio::test]
async fn transition_pending_to_approved_sets_approver_and_resolved_at() {
    let repo = FakeConversionRepo::new();
    let new = new_pending(0xAA, 0x11, 0x22);
    ...
    let updated = repo
        .transition_pending_to_approved(&scope(), inserted.id, approver, when)
        .await
        .expect("approve");
...
async fn transition_pending_to_cancelled_on_approved_returns_already_resolved() {
    ...
    let approver = Uuid::from_u128(APPROVER);
    let when = fixed_now() + Duration::minutes(5);
    repo.transition_pending_to_approved(&scope(), inserted.id, approver, when)
        .await
        .expect("approve");
    let err = repo
