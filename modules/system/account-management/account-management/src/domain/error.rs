...
//!
//! Implements the authoritative 8-category / N-sub-code public error contract
...
/// The 8 stable public error categories enumerated by PRD §5.8.
///
...
pub enum AmError {
    ...
    ///
    /// Surfaces as `conflict` (HTTP 409, `sub_code = serialization_conflict`)
    ...
    /// `sub_code` so retry-exhaustion is distinguishable from
...
impl AmError {
    ...
    /// Stable public `sub_code` exposed through the Problem envelope per
    /// DESIGN §3.8 / `OpenAPI` `Problem.sub_code`.
    #[must_use]
    pub const fn sub_code(&self) -> &'static str {
        match self {
...
mod tests {
    ...
    fn http_status_mapping_matches_design_3_8() {
        ...
        // `internal` (HTTP 500). The finer-grained `sub_code` stays
        ...
        assert_eq!(serialization_conflict.sub_code(), "serialization_conflict");
    }

    #[test]
    fn sub_code_tokens_match_openapi_problem_enum() {
        // Acceptance criterion §6: every stable public `sub_code` from DESIGN
        ...
        let cases: &[(AmError, &str)] = &[
            ...
            (
                AmError::SerializationConflict { detail: s },
                "serialization_conflict",
            ),
        ];
        for (err, expected) in cases {
            assert_eq!(err.sub_code(), *expected, "variant {err:?}");
        }
    }
    ...
    fn enforcer_error_conversion_preserves_source_chain() {
        ...
        for (source, category, sub_code, source_message) in cases {
            let err = AmError::from(source);
            assert_eq!(err.category(), category, "converted error: {err:?}");
            assert_eq!(err.sub_code(), sub_code, "converted error: {err:?}");
