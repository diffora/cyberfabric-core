...
mod tests {
    ...
    #[test]
    fn child_list_query_rejects_provisioning_status_token() {
        // Pin the wire-side half of the provisioning-row visibility
        // contract documented on `TenantStatus` (lines 23-27): the public
        // enum has no `Provisioning` variant, so callers must not be
        // able to smuggle the SDK-invisible status through the
        // `?status=` query parameter. The repo-side filter
        // (`infra/storage/repo_impl.rs:1611-1623`,
        // `WHERE status IN (1,2,3)`) is the second line of defence; if
        // either gate is removed, concurrent provisioning rows would
        // leak through `listChildren` mid-saga.
        for token in ["provisioning", "PROVISIONING", "Provisioning"] {
            let q = ChildListQuery {
                top: None,
                skip: None,
            ...
            let err = q
                .into_domain_query(Uuid::nil(), 200)
                .expect_err("provisioning token must reject");
            assert_eq!(err.code(), "validation", "token={token}");
        }
    }

    #[test]
    fn child_list_query_default_status_filter_is_none_repo_excludes_provisioning() {
        // When the caller passes no `?status=` filter, the wire layer
        // emits a `ListChildrenQuery` with `status_filter() == None`.
        // The repo-side default in `infra/storage/repo_impl.rs:
        // 1611-1623` then constrains the SQL to
        // `WHERE status IN (active, suspended)` — `Provisioning`
        // (status=0) and `Deleted` (status=3) are both excluded. This
        // test pins the wire-side shape; the SQL-side test is
        // deferred to the integration scaffold.
        let q = ChildListQuery {
            top: None,
            skip: None,
            status: None,
        ...
        let dq = q.into_domain_query(Uuid::nil(), 200).expect("ok");
        assert!(
            dq.status_filter().is_none(),
            "no `?status=` ⇒ no domain filter (repo applies the default)"
        );
    }

    #[test]
    fn tenant_status_dto_has_no_provisioning_variant() {
        // Belt-and-braces guard against a future patch reintroducing a
        // `provisioning` wire variant. If someone adds
        // `TenantStatus::Provisioning`, this round-trip would surface
        // it and break this assertion. The `provisioning` token must
        // not deserialize into the wire enum.
        let result: Result<TenantStatus, _> = serde_json::from_str("\"provisioning\"");
        assert!(
            result.is_err(),
            "wire enum must not accept `provisioning`; got: {result:?}"
        );
    }
}
