...
/// the `sub_code` field stable across all handlers so dashboards stay
/// uniform.
fn log_handler_error(op: &'static str, err: &AmError) {
    if err.http_status() >= 500 {
        error!(target: "am.rest", op, sub_code = %err.sub_code(), "{op} failed");
    } else {
        warn!(target: "am.rest", op, sub_code = %err.sub_code(), "{op} failed");
    }
}
...
/// helper delegates to the already-stable `http_status()` /
/// `category().as_str()` / `sub_code()` accessors and emits `sub_code`
/// as a top-level Problem field per the public OpenAPI contract.
...
pub fn am_error_to_problem(err: &AmError) -> Problem {
    ...
    let mut problem = Problem::new(status, title_for(err), detail)
        .with_type(format!("https://errors.hyperspot.com/am/{sub_code}"))
        .with_code(code)
        .with_sub_code(sub_code);
...
mod tests {
    ...
    async fn create_tenant_maps_idp_unavailable_to_503_idp_unavailable() {
        ...
        assert_eq!(payload["sub_code"], "idp_unavailable");
    }

    #[test]
    fn problem_serializes_sub_code_as_top_level_field() {
        ...
        assert_eq!(encoded["sub_code"], "serialization_conflict");
        assert!(
            encoded.get("context").is_none(),
            "sub_code must not be nested under context: {encoded}"
        );
    }
    ...
    async fn create_tenant_maps_idp_unsupported_operation_to_501() {
        ...
        assert_eq!(payload["sub_code"], "idp_unsupported_operation");
    }
    ...
    async fn update_tenant_maps_validation_to_422_validation() {
        ...
        assert_eq!(payload["sub_code"], "validation");
    }
    ...
    async fn get_tenant_not_found_maps_to_404_not_found() {
        ...
        assert_eq!(payload["sub_code"], "not_found");
    }
    ...
    async fn create_tenant_maps_tenant_depth_exceeded_to_409() {
        ...
        assert_eq!(problem.sub_code, "tenant_depth_exceeded");
    }
    ...
    async fn am_error_mapping_is_total_over_variants() {
        let cases: &[(AmError, http::StatusCode, &str, &str)] = &[
            (
    ...
    async fn delete_tenant_root_returns_422_root_tenant_cannot_delete() {
        ...
        assert_eq!(resp.status(), AxumStatus::UNPROCESSABLE_ENTITY);
        let payload = read_body(resp.into_body()).await;
        assert_eq!(payload["sub_code"], "root_tenant_cannot_delete");
    }
    ...
    async fn get_tenant_cross_tenant_returns_403_cross_tenant_denied() {
        ...
        let payload = read_body(resp.into_body()).await;
        assert_eq!(payload["sub_code"], "cross_tenant_denied");
    }
