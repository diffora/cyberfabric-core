...
pub struct AccountManagementConfig {
    ...
    /// Hard cap on the number of `tenants` rows the
    /// `check_hierarchy_integrity(Whole)` checker is allowed to load
    /// into memory. When the live row count exceeds this value the
    /// checker returns `AmError::Internal` with a structured
    /// `integrity scope too large` diagnostic and the operator is
    /// expected to drive the audit through the `Subtree` scope
    /// instead. Default `50_000`.
    pub integrity_max_tenants: usize,
...
mod tests {
    ...
    fn deserializes_empty_object_as_default() {
        ...
        assert_eq!(cfg.integrity_max_tenants, 50_000);
