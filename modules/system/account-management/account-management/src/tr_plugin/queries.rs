...
//!   follow-up).
//! - Uses [`plugin_scope`] (a thin named wrapper over
//!   [`AccessScope::allow_all`]) on every secure read. AM's `tenants` /
...
//!   trusted" (DESIGN §4.2): the gateway authenticates the caller and
//!   decides whether to forward; the plugin reads unconditionally.
//!   Centralizing the scope through a single named helper gives us
//!   one greppable audit point for the trust elevation rather than
//!   eight bare `AccessScope::allow_all()` constructions scattered
//!   across the file.
...
//!   `get_type_schemas_by_uuid` (page-style results) — the call
//!   shape mirrors `domain::tenant::service::lower_to_tenant_info` /
//!   `lower_to_tenant_page`, but the failure policy is stricter:
//!   per DESIGN §3.4 / §5, **any** registry failure (single-row
//!   miss or even one per-uuid error in a batch) fails the SDK
//!   call with [`TenantResolverError::Internal`]; the plugin must
//!   not return raw UUIDs in place of the public chained
//!   `tenant_type`. The `tenant_type: None` graceful-degradation
//!   path used by AM's own service is intentionally NOT applied
//!   here.
...
/// The `AccessScope` every secure read in this module runs under.
///
/// Returns [`AccessScope::allow_all`]: AM's `tenants` / `tenant_closure`
/// entities are declared `no_tenant, no_resource, no_owner, no_type`,
/// so this scope adds no implicit `WHERE` and is functionally a no-op
/// at the row predicate. The trust rationale (DESIGN §4.2 — gateway is
/// trusted) lives on the module docstring; this helper exists so the
/// trust elevation has exactly one named call site that audits / greps
/// can point at, rather than eight bare `AccessScope::allow_all()`
/// constructions that look indistinguishable from a copy-paste mistake.
fn plugin_scope() -> AccessScope {
    AccessScope::allow_all()
}

/// Emit a structured audit event when a caller selects
/// [`BarrierMode::Ignore`] on a hierarchy read.
///
/// Per FEATURE §6 (`cpt-cf-tr-plugin-nfr-audit-trail`) and DESIGN §4
/// the plugin MUST surface a per-call audit signal whenever the
/// barrier predicate is bypassed; the gateway is trusted to gate the
/// `Ignore` mode at the public API surface, but the plugin still
/// records the bypass so operator audit can correlate authz
...
///
/// No-op when `mode` is [`BarrierMode::Respect`].
fn audit_barrier_bypass(
    ...
    if !matches!(mode, BarrierMode::Ignore) {
        return;
    }
    match extra {
...
pub(super) async fn get_root_tenant(
    ...
    let mut roots = tenants::Entity::find()
        ...
        .await
        .map_err(|e| scope_err_to_tr_err(&e))?;
    // Drop the borrowed `DbConn` before any registry round-trip in
    // the matched arms below; symmetric with the explicit drops in
    // `get_tenants`/`get_ancestors`/`get_descendants` (note:
    // `get_tenant` itself has no `conn` binding — it goes through
    // `read_tenant_visible` which scopes its own connection).
    // `modkit_db::DbConn` is a single-owner handle (not `Clone`,
    // not `Arc`-backed), so this provably releases the borrow.
    let _ = conn;
...
pub(super) async fn get_tenants(
    ...
    let rows = tenants::Entity::find()
        ...
        .await
        .map_err(|e| scope_err_to_tr_err(&e))?;
    // Explicit drop of the borrowed `DbConn` before
    // `resolve_tenant_types_for_rows` (which goes off-DB to the
    // registry); symmetric with the same pattern in
    // `get_ancestors` and `get_descendants`. `modkit_db::DbConn`
    // is a single-owner handle (not `Clone`, not `Arc`-backed) so
    // this provably releases the borrow at this point.
    let _ = conn;
...
pub(super) async fn get_ancestors(
    ...
    // Drop the borrowed `DbConn` before calling `read_tenants_visible_bulk`,
    // which acquires its own connection internally. `modkit_db::DbConn` is a
    // single-owner handle (not `Clone`, not `Arc`-backed), so the explicit
    // `let _ = conn` provably releases the underlying borrow at this point —
    // a refactor that ever changes `DbConn` to `Clone` would need to revisit
    // every such drop site in this file.
    let _ = conn;

    // Hydrate ancestors. Per AM's invariant
    ...
    // — closure references a non-existent or provisioning tenant
    // — therefore signals storage corruption and we fail closed
    // with `Internal` rather than silently truncating the chain
    // (which would be invisible to the caller and could be
    // mistaken for a legitimate empty result).
    ...
    // as tie-break. The DESC-on-depth → "direct parent first, root last"
    // mapping holds because AM's `tenants.depth` is the **root-relative**
    // depth: 0 for the root, 1 for direct children, etc. — invariant
    // enforced by the writer in `domain::tenant::service` on every insert.
    // If AM ever permits reparenting (which would mutate `depth`), this
    // ordering — and the doc claim above — both need to be revisited.
    // Ordering happens here because we hydrated through a bulk-by-id
    // read instead of a JOIN; the two-call shape is what the
    // secure-extension's per-entity contract permits today.
...
pub(super) async fn get_descendants(
    ...
    let starting = read_tenant_visible(db, id.0)
        .await?
        .ok_or(TenantResolverError::TenantNotFound { tenant_id: id })?;
    // Emit the bypass audit AFTER the visibility probe so the
    // audit stream records only requests that actually proceed
    // (not those rejected with `TenantNotFound`).
    ...
    //
    // `max_depth` is **not** pushed into this scan: AM's
    // `tenant_closure` carries no per-pivot depth column, and
    // `tenants.depth` is root-relative (not pivot-relative), so a
    // server-side depth filter is not expressible without the
    // recursive-CTE optimization tracked in the module-level doc.
    // For a `max_depth = Some(N)` call against a wide subtree, the
    // current implementation reads the full barrier-bounded subtree
    // and trims at emission time. Cost is bounded by the subtree
    // size, not by `N`; the recursive-CTE follow-up reduces this to
    // depth-N reads.
    let mut closure_filter = Condition::all()
    ...
    // `modkit_db::DbConn` is a single-owner handle (not `Clone`, not
    // `Arc`-backed), so the explicit `let _ = conn` provably releases
    // the borrow at this point — symmetric with the same drop site
    // in `get_ancestors` above.
    let _ = conn;
    ...
    // Per AM's invariant every closure row's `descendant_id` must
    // resolve to a non-provisioning `tenants` row (the closure
    // CHECK constraint pins `descendant_status ∈ {1,2,3}` and
    // provisioning tenants own no closure rows in either direction).
    // A hydration miss therefore signals storage corruption — fail
    // closed with `Internal` rather than silently truncating the
    // emitted subtree. Symmetric with the same defense in
    // `get_ancestors` above.
    ...
    if hydrated_ids != closure_ids {
        ...
        tracing::warn!(
            ...
            pivot = %id.0,
            missing = ?missing,
            "tenant_closure references descendant ids that did not resolve to visible tenants rows; \
             AM hierarchy invariant violated"
        );
        return Err(TenantResolverError::Internal(
            "tenant resolver hierarchy invariant violated".to_owned(),
    ...
    //
    // **Fail-closed on corruption:** `visited` provides both cycle
    // protection AND completeness verification. A revisit indicates
    // a `parent_id` cycle (AM invariant violation); we surface
    // `Internal` rather than `warn!+continue`-ing because silent
    // continuation could mask half a cycle's worth of nodes from
    // the result. After the walk, we additionally verify that the
    // visited set covers every closure descendant from `row_by_id` —
    // a closure-row descendant whose `parent_id` chain never
    // reaches `id.0` is the second corruption shape (the closure
    // says it's in the subtree, but the parent-id graph says
    // otherwise) and likewise surfaces `Internal`. The starting
    // tenant is pre-marked so a back-edge to it from some
    // descendant cannot revisit.
    //
    // The `max_depth` bound deliberately does NOT prune the
    // completeness check: if `max_depth = Some(N)` is small enough
    // that some closure descendants at deeper levels are not
    // reached by the walk, those descendants are still valid
    // members of the subtree and not corruption. We model this by
    // skipping the completeness check when `max_depth` is bounded —
    // bounded walks are only run with the corruption defense
    // delegated to the closure-vs-tenants hydration check above.
    ...
    while let Some((node_id, depth)) = stack.pop() {
        ...
        let Some(row) = row_by_id.get(&node_id) else {
            // Closure says this node is a descendant of pivot but
            // the bulk read did not produce its `tenants` row. The
            // hydrated-vs-closure check above should have caught
            // this; reaching here means a fresh corruption between
            // the two reads — fail closed.
            tracing::warn!(
                target: "tr_plugin",
                tenant_id = %node_id,
                pivot = %id.0,
                "pre-order walk reached tenant id absent from row_by_id; hierarchy invariant violated"
            );
            return Err(TenantResolverError::Internal(
                "tenant resolver hierarchy invariant violated".to_owned(),
            ));
    ...
    // Completeness check: every closure descendant within the
    // requested depth bound must be reachable from `id.0` through
    // the `parent_id` graph. A discrepancy means closure asserts X
    // is a descendant of pivot but the parent-id walk disagrees —
    // fail closed. The check is bounded by `max_depth` so closure
    // descendants intentionally outside the walk envelope (legit
    // depth-bound trim) are not flagged as corruption.
    //
    // Bounding uses AM's `tenants.depth` (root-relative absolute
    // depth) — `expected_max_depth = starting.depth + max_depth`
    // for `Some(N)`, no upper bound for `None`. The walk visits
    // nodes up to depth `N+1` (it inserts into `visited` and then
    // skips processing on `> N`), but for the completeness check
    // we only require nodes at depth `≤ N` to be visited;
    // deeper-but-in-closure nodes are out of scope.
    let bound_inclusive: Option<i32> = match max_depth {
        Some(n) => Some(starting.depth.saturating_add(i32::try_from(n).unwrap_or(i32::MAX))),
        None => None,
    };
    let unreached: Vec<Uuid> = row_by_id
        .iter()
        .filter(|(closure_id, row)| {
            !visited.contains(*closure_id)
                && bound_inclusive.is_none_or(|cap| row.depth <= cap)
        })
