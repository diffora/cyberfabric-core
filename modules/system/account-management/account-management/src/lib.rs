...
//!
//! * the stable public error taxonomy (8 categories × DESIGN §3.8 codes)
...
// `build_activation_rows`, `is_due`, `order_batch_leaf_first`, and
// `compute_next_backoff` are not re-exported at the crate root: they
// are domain-internal helpers with no external consumers, reachable
// via `crate::domain::{tenant::{closure,retention},util::backoff}`
// for tests inside this crate. The previous in-memory integrity
// classifiers were removed in the SQL-side classifier refactor; their
// categories now originate from 10 fixed-shape SQL queries inside the
// storage adapter (see `feature-tenant-hierarchy-management.md`
// "Removed Surface").
...
// `TenantRepoImpl` + `AmDbProvider` are re-exported behind `#[doc(hidden)]`
// solely for the in-tree integration test harness (`tests/integrity_pg_integration.rs`).
// Production code MUST go through the `module::AccountManagementModule`
// initializer; the storage adapter is not part of the stable public API.
#[doc(hidden)]
pub use infra::storage::migrations::Migrator as TestMigrator;
#[doc(hidden)]
