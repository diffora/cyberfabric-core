...
use crate::infra::observability::{AmMetricsMeter, AmMetricsPort, install_metrics_port};
...
impl AccountManagementModule {
    ...
    /// # Lifecycle ordering
    ///
    /// Sibling features MUST register hooks during their own
    /// `init` — i.e. before this module's `serve` is invoked. Once
    /// `serve` starts the retention + reaper tick loops, hooks
    /// registered later may race with an in-flight `hard_delete_one`
    /// call: the hook list is snapshotted per tick, so a
    /// late-arriving hook may be observed by some concurrent tenants
    /// but not others. The ModKit runtime orders `init`s before
    /// `serve`, so this is naturally satisfied as long as no
    /// feature defers its `register_hard_delete_hook` call into a
    /// background task or its own `serve` entry-point.
    ///
    /// # Errors
    ///
...
impl Module for AccountManagementModule {
    ...
    async fn init(&self, ctx: &ModuleCtx) -> anyhow::Result<()> {
        ...
        // Wire the typed OpenTelemetry meter behind `AmMetricsPort`
        // before the bootstrap saga or service construction can run —
        // emit_metric calls from those paths must land on the real
        // meter, not the default `NoopMetricsPort`. The call sequence
        // here is exactly: scope → meter → AmMetricsMeter::new →
        // install_metrics_port (idempotent first-set-wins).
        let scope =
...
mod tests {
    ...
    async fn inert_rg_binding_emits_warning_marker_and_dependency_gauge() {
        ...
        assert_eq!(
            checker
                .count_ownership_links(
                    &modkit_security::SecurityContext::anonymous(),
