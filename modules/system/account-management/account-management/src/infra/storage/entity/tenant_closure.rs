...
//!
//! Mirrors `migrations/0001_create_tenants.sql` column-for-column and
//! enforces the barrier + SDK-visible `descendant_status` invariants through
//! the DB-level `CHECK` constraints in the migration. Phase 1 scope:
//! entity definition only; closure maintenance helpers that build the rows
//! live in `domain/tenant/closure.rs`.
//!
