...
//! [`account_management_sdk::ProvisionResult::metadata`] keyed by
//! `tenant_id` (one row per tenant) and replays it on every subsequent
//! `IdpPluginClient` call for that tenant via
//! [`account_management_sdk::TenantContext::metadata`] and
//! [`account_management_sdk::DeprovisionTenantRequest::tenant_context`].
//!
