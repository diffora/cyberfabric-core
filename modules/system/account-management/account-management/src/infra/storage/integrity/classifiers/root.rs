...
//!
//! Subtree audits (`scope_root.is_some()`) skip this check — by
//! construction the subtree starts from a single in-tree pivot, so the
//! global root-count question is meaningless inside the subtree.
...
mod tests {
    ...
    #[test]
    fn subtree_scope_skips_root_check() {
        let snap = Snapshot::new(vec![t(1, None), t(2, None)], vec![]);
        let v = classify(&snap, Some(Uuid::from_u128(1)));
        assert!(v.is_empty(), "subtree audit MUST skip root-count check");
    }
}
