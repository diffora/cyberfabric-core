...
pub(super) fn classify(snap: &Snapshot, scope_root: Option<Uuid>) -> Vec<Violation> {
    ...
    for row in snap.closure() {
        // Subtree-scope filter: skip closure rows whose descendant is
        // not in the audited subtree. Mirrors the seven sibling
        // classifiers; without it a `Subtree(root)` audit would emit
        // violations for tenants entirely outside the requested
        // subtree.
        if !snap.is_in_scope(scope_root, row.descendant_id) {
            continue;
        }
