...
use crate::domain::tenant::integrity::{IntegrityCategory, RepairReport};
...
#[must_use]
pub fn compute_repair_plan(snap: &Snapshot) -> RepairPlan {
    ...
    let deferred_per_category = compute_deferred_counts(snap);
...
fn compute_deferred_counts(snap: &Snapshot) -> Vec<(IntegrityCategory, usize)> {
    use super::classifiers;
    let report = classifiers::run_non_derivable(snap);
