...
fn intermediate_self_managed_ancestor_locks_barrier_above_only() {
    ...
    assert!(
        classify(&snap_consistent, None).is_empty(),
    ...
    let v = classify(&snap_divergent, None);
    assert_eq!(v.len(), 1);
...
fn self_rows_are_not_checked_for_barrier_divergence() {
    // (1,1) carries barrier=0 trivially; even if descendant_status
    // matches, no barrier violation should be raised.
    let snap = Snapshot::new(
        vec![t(1, None, true, TenantStatus::Active)],
        vec![c(1, 1, 0, TenantStatus::Active)],
    );
    assert!(classify(&snap, None).is_empty());
}
