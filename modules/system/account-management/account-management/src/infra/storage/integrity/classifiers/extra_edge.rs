...
pub(super) fn classify(snap: &Snapshot, scope_root: Option<Uuid>) -> Vec<Violation> {
    ...
    // Subtree-scope filter: report a closure row only when its
    // descendant is in the audited subtree. Missing descendants are
    // never in scope (they're not in `tenants`), so they're skipped
    // under `Subtree`; whole-tree audits (`scope_root == None`) keep
    // the legacy "report every stale row" semantics.
    let in_scope = |descendant_id: Uuid| snap.is_in_scope(scope_root, descendant_id);
