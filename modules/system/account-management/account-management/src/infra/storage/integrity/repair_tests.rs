...
fn cycle_tail_is_skipped_by_planner() {
    ...
    let plan = compute_repair_plan(&snap, None);
    assert!(
...
fn missing_self_row_repair_idempotent_on_clean_input() {
    let plan = compute_repair_plan(&clean_chain_1_2_3(TenantStatus::Active), None);
    assert_eq!(plan.total_ops(), 0);
}
...
fn repair_then_repair_is_noop() {
    ...
    let plan = compute_repair_plan(&snap_before, None);
    ...
    let plan_after = compute_repair_plan(&snap_after, None);
    assert_eq!(
...
// ---------------------------------------------------------------------
// Subtree scope — only descendants in scope are repaired.
// ---------------------------------------------------------------------

#[test]
fn subtree_scope_skips_out_of_scope_descendants() {
    // 1 -> 2; 3 is a separate root.
    // Tenant 3 is missing its self-row but is OUT of subtree(1).
    let snap = Snapshot::new(
        vec![
            t(1, None, TenantStatus::Active, false),
            t(2, Some(1), TenantStatus::Active, false),
        ...
        vec![
            ...
            c(2, 2, 0, TenantStatus::Active),
            c(1, 2, 0, TenantStatus::Active),
            // (3,3) self-row MISSING — but 3 is out of subtree(1).
        ],
    );
    let plan = compute_repair_plan(&snap, Some(Uuid::from_u128(1)));
    assert_eq!(
        plan.total_ops(),
        0,
        "out-of-scope descendant must not be repaired"
    );
}
