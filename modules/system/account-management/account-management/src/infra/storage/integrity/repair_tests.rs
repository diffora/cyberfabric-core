...
fn cycle_tail_is_skipped_by_planner() {
    ...
    let plan = compute_repair_plan(&snap);
    assert!(
...
fn missing_self_row_repair_idempotent_on_clean_input() {
    let plan = compute_repair_plan(&clean_chain_1_2_3(TenantStatus::Active));
    assert_eq!(plan.total_ops(), 0);
}
...
fn repair_then_repair_is_noop() {
    ...
    let plan = compute_repair_plan(&snap_before);
    ...
    let plan_after = compute_repair_plan(&snap_after);
    assert_eq!(
