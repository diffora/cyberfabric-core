    ...
    let result = with_serializable_retry(&repo.db, move || {
        ...
        Box::new(move |tx: &DbTx<'_>| {
            Box::pin(async move {
                ...
                // (`ProvisionResult::metadata = None`); we still write
                ...
                // distinction. Saga step 3 is the only writer; the
                // SERIALIZABLE retry boundary ensures activation is
                // the unique source of the initial row.
