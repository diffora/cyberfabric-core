        ...
        Box::new(move |tx: &DbTx<'_>| {
            Box::pin(async move {
                ...
                let parent_row = tenants::Entity::find()
                    ...
                    .filter(
                        Condition::all()
                            .add(tenants::Column::Id.eq(parent_id_from_req))
                    ...
                    .ok_or_else(|| {
                        TxError::Domain(DomainError::Internal {
                            diagnostic: format!(
                                "conversion {}: parent tenant {parent_id_from_req} disappeared \
                ...
                if !descendants_of_target.is_empty() {
                    // TODO(scale): unbounded `IN (...)` over the
                    // descendant id list. SeaORM emits one bind
                    // parameter per id; on a wide subtree this can
                    // approach the Postgres 65 535-parameter ceiling
                    // mid-transaction and turn a recoverable approve
                    // into a hard failure. Replace with a self-join
                    // on `tenant_closure` (descendants by ancestor =
                    // target) once #1813 lands so the engine handles
                    // the cardinality. Until then a corruption-free
                    // single conversion stays well under the cap, but
                    // a bulk-converted root subtree would not.
                    let candidate_rows = tenant_closure::Entity::find()
                        .secure()
                        .scope_with(&AccessScope::allow_all())
                        .filter(
                            Condition::all().add(
                                tenant_closure::Column::DescendantId
                                    .is_in(descendants_of_target.clone()),
                            ),
                        )
                        .all(tx)
                        .await
...
/// return `true` iff the `target` tenant appears on it. Cycle-safe via a
/// hop cap matching the AM hierarchy budget.
    ...
    let candidate_ids: Vec<Uuid> = conversion_requests::Entity::find()
        ...
        .filter(
            Condition::all()
                ...
                .add(conversion_requests::Column::ResolvedAt.lte(cutoff))
                .add(conversion_requests::Column::DeletedAt.is_null()),
        )
