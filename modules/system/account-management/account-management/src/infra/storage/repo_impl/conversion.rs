...
//! [`AccessScope::allow_all`] (the `conversion_requests` entity is
//! declared `Scopable(tenant_col = "tenant_id", no_resource, no_owner,
//! no_type)` as forward-compat for the post-#1813 REST surface, but
//! dual-consent flows still need barrier penetration on self-managed
//! children so the repo deliberately passes `allow_all` rather than
//! the caller's narrowed scope; authorization is enforced one layer
//! up at the service-level dual-consent role check), and DB errors
//! are routed through the canonical-mapping classifier so domain code
//! never sees a raw `DbErr`.
//!
...
///
/// The `tenant_type_checker` is wired in for the
/// [`ConversionRepo::apply_conversion_approval`] seam — type
/// re-evaluation runs inside the same transaction as the
/// `tenants.self_managed` flip and the closure-barrier rewrite. The
/// production wiring resolves the GTS-backed checker; tests pass an
/// inert one.
...
impl ConversionRepoImpl {
    /// Build a new repo adapter over the shared AM DB provider plus a
    /// [`TenantTypeChecker`] used by the apply-approval seam.
    #[must_use]
    pub fn new(
        db: Arc<AmDbProvider>,
        tenant_type_checker: Arc<dyn TenantTypeChecker + Send + Sync>,
...
async fn insert_pending(
    ...
    let insert_res = conversion_requests::Entity::insert(am)
        .secure()
        // `conversion_requests` is declared `no_tenant, no_resource`
        // (see entity doc) so the `InTenantSubtree` predicate has no
        // resolvable property to clamp on. `scope_unchecked` mirrors
        // the `tenants` INSERT posture in
        // `repo_impl::lifecycle::insert_provisioning`. Authorization
        // for the operation as a whole is enforced upstream at the
        // PDP gate in the service layer (dual-consent role check on
        // the caller-supplied `ConversionCaller`).
...
async fn transition_pending_to_approved(
    repo: &ConversionRepoImpl,
    scope: &AccessScope,
    request_id: Uuid,
    approved_by: Uuid,
...
async fn apply_conversion_approval(
    ...
    let checker = Arc::clone(&repo.tenant_type_checker);
    with_serializable_retry(&repo.db, move || {
        ...
        Box::new(move |tx: &DbTx<'_>| {
            Box::pin(async move {
                ...
                // 3. Re-load the parent tenant for the type re-eval.
                //
                ...
                // 4. Type re-evaluation — runs BEFORE any flip /
                //    barrier rewrite per the apply algorithm. A
                //    rejection aborts the whole TX and leaves the
                //    pending row + tenant flag + closure barriers
                //    untouched.
                checker
                    .check_parent_child(parent_row.tenant_type_uuid, tenant_row.tenant_type_uuid)
                    .await
                    .map_err(TxError::Domain)?;

                // 5. Flip `tenants.self_managed`.
                //
                // The UPDATE filter pins both the tenant id AND
                // `status = Active` so a peer that committed a
                // suspend / soft-delete between the step-2 precheck
                // and this point cannot have its tenant silently
                // flipped under us. On `rows_affected == 0` we surface
                // `Validation` (status changed under our feet) — the
                // caller's apply rolls back, the pending row remains
                // for a retry once the operator reactivates the
                // tenant. `updated_at` is stamped from the service-
                ...
                if flip_res.rows_affected == 0 {
                    return Err(TxError::Domain(DomainError::Validation {
                        detail: format!(
                            "tenant {} is no longer active under TX (status changed between \
                             precheck and flip)",
                ...
                // 6. Recompute closure-row barriers for every row
                ...
                // step-5 `tenants.self_managed` flip above —
                ...
                // 7. Stamp the request transition LAST so any earlier
...
impl ConversionRepo for ConversionRepoImpl {
    ...
    async fn transition_pending_to_approved(
        ...
        transition_pending_to_approved(self, scope, request_id, approved_by, resolved_at).await
    }
