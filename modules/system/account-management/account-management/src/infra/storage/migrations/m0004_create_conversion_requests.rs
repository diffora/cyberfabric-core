//! Migration `m0004` — create the `conversion_requests` table backing the
//! managed/self-managed dual-consent conversion flow.
//!
//! The table records the durable state machine for post-creation mode
//! changes (`pending` -> `approved` | `cancelled` | `rejected` | `expired`)
//! and is wired into the rest of the schema via `FOREIGN KEY` constraints
//! on `tenants.id` for both the converting tenant (`tenant_id`) and the
//! parent linkage (`parent_id`). Both FKs declare `ON DELETE CASCADE` so
//! conversion-request rows cannot outlive the tenants they reference and
//! so the existing tenant retention sweep is sufficient to clean them up
//! without a separate hard-delete pass on `conversion_requests`.
//!
//! Backend coverage mirrors `m0001`: per-backend raw DDL is used so the
//! `CHECK` invariants and the partial unique index that enforces the
//! at-most-one-pending invariant survive byte-for-byte on Postgres, with a
//! dialect-equivalent expression on `SQLite`. `MySQL` is unsupported and
//! returns `DbErr::Custom(MYSQL_NOT_SUPPORTED)`.
//!
//! `down` drops the table only — both the FK constraints (Postgres) and
//! the indexes drop together with the table on each supported dialect.
...
CREATE TABLE IF NOT EXISTS conversion_requests (
    ...
    --   * status=0 (pending):   all *_by NULL,            resolved_at NULL
...
CREATE TABLE IF NOT EXISTS conversion_requests (
    ...
    -- predicate over `(status, *_by, resolved_at)` works on `SQLite`
    -- because every column referenced is portable across both
    -- backends (`SMALLINT`/`TEXT IS [NOT] NULL`).
    CONSTRAINT ck_conversion_requests_actor_invariant
        CHECK (
            (status = 0 AND approved_by IS NULL AND cancelled_by IS NULL
                AND rejected_by IS NULL AND resolved_at IS NULL)
    ...
    FOREIGN KEY (parent_id) REFERENCES tenants(id)
        ON UPDATE CASCADE ON DELETE CASCADE
                ...
                ",
                // The PG version uses a partial unique index on
                // `(tenant_id) WHERE status = 0 AND deleted_at IS NULL`;
                // a single `pending` row per tenant collapses cleanly
                // because `tenant_id` is `NOT NULL`. SQLite (per SQL
                // standard) treats NULLs in a unique index as distinct,
                // so any nullable column landing in the indexed key
                // would defeat the partial filter. `tenant_id` is
                // non-null here, so on `SQLite` we additionally fold
                // `deleted_at` into the key via `COALESCE(deleted_at,
                // '')` to mirror the `m0001` `ux_tenants_single_root`
                // pattern and guarantee NULL-distinct semantics never
                // bypass the constraint.
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_conversion_requests_pending ON conversion_requests (tenant_id, COALESCE(deleted_at, '')) WHERE status = 0 AND deleted_at IS NULL;",
                "CREATE INDEX IF NOT EXISTS idx_conversion_requests_tenant_status ON conversion_requests (tenant_id, status);",
                "CREATE INDEX IF NOT EXISTS idx_conversion_requests_parent_status ON conversion_requests (parent_id, status) WHERE parent_id IS NOT NULL;",
                "CREATE INDEX IF NOT EXISTS idx_conversion_requests_expiry_sweep ON conversion_requests (expires_at) WHERE status = 0 AND deleted_at IS NULL;",
                "CREATE INDEX IF NOT EXISTS idx_conversion_requests_retention_scan ON conversion_requests (resolved_at) WHERE status IN (1, 2, 3, 4) AND deleted_at IS NULL;",
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE IF EXISTS conversion_requests;")
            .await?;
        Ok(())
    }
}
