...
impl GtsTenantTypeChecker {
    ...
    /// Returns `None` if the key is missing, `null`, not an array, or
    /// contains a non-string item — every malformed shape collapses to
    /// `InvalidTenantType` at the call site.
