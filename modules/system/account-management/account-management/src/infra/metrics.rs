...
use crate::domain::metrics::{
    ...
    AM_HIERARCHY_INTEGRITY_REPAIRED, AM_HIERARCHY_INTEGRITY_REPAIR_RUNS,
    AM_HIERARCHY_INTEGRITY_RUNS, AM_HIERARCHY_INTEGRITY_VIOLATIONS, AM_INTEGRITY_LOCK_EVENTS,
    AM_METADATA_RESOLUTION, AM_RETENTION_INVALID_WINDOW, AM_SERIALIZABLE_RETRY, AM_TENANT_RETENTION,
...
impl IntegrityMetricsPort for AmMetricsMeter {
    ...
    fn hierarchy_integrity_last_failure(
        &self,
        outcome: IntegrityRunOutcome,
        epoch_seconds: i64,
    ) {
        self.hierarchy_integrity_last_failure.record(
            epoch_seconds,
            &[KeyValue::new("outcome", outcome.as_str())],
        );
    }
...
impl MetricsFacadeBridge for AmMetricsMeter {
    fn emit(&self, family: &'static str, kind: MetricKind, labels: &[(&'static str, &str)]) {
        ...
        match family {
            ...
            _ => {
                debug_assert!(false, "emit_metric called with unknown counter family {family}");
            }
        }
    }
    ...
    fn emit_gauge(&self, family: &'static str, value: i64, labels: &[(&'static str, &str)]) {
        ...
        match family {
            ...
            _ => {
                debug_assert!(false, "emit_gauge_value called with unknown gauge family {family}");
            }
        }
    }
