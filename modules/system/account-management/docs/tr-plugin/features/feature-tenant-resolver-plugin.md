...
- Database connection failure, query timeout, or tenant-type reverse-hydration failure — plugin returns the canonical `service_unavailable` code (mapped to `TenantResolverError::Internal`); the gateway decides retry.
...
4. [ ] - `p1` - **IF** no row is returned (no non-provisioning root present, including bootstrap window) - `inst-flow-get-root-tenant-none-branch`
   1. [ ] - `p1` - **RETURN** the canonical `service_unavailable` code to the gateway - `inst-flow-get-root-tenant-return-internal-none`
5. [ ] - `p1` - **ELSE IF** more than one root candidate is returned (single-root invariant violated) - `inst-flow-get-root-tenant-multiple-branch`
   1. [ ] - `p1` - **RETURN** the canonical `service_unavailable` code to the gateway - `inst-flow-get-root-tenant-return-internal-multiple`
...
- Error taxonomy: delegated to `cpt-cf-account-management-feature-errors-observability` (catalog owner of all `code` and `code` identifiers and their HTTP status mapping)
...
- Sibling integration: `cpt-cf-account-management-feature-errors-observability` (canonical catalog owner of `code`, `code`, HTTP status mapping, and Problem envelope shape)
