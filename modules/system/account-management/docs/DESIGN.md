...
- IdP integration: execute tenant deprovisioning during hard deletion and provide IdP user operations (provision, deprovision, query) through `IdpPluginClient`. Providers **MUST NOT** silently no-op on mutating operations; failures are retried rather than skipped.
...
`IdpPluginClient` is the deployment-specific outbound identity boundary for tenant provisioning, tenant deprovisioning, user lifecycle operations, and an explicit bootstrap readiness probe (`check_availability`). The architecture expects:
...
| _(deployment prerequisite)_ | — | — | All metadata schemas that the IdP provider may return in `ProvisionResult` entries must be pre-registered in GTS before bootstrap runs. AM validates all persisted metadata against GTS schemas; unregistered `schema_id`s are rejected with `not_found`, causing the saga finalization step to fail and the provisioning reaper to compensate. (`root_tenant_metadata` is **opaque to AM by default** — forwarded as-is to the provider plugin with no semantic interpretation. Plugins MAY opt in to structural pre-validation by overriding `IdpPluginClient::provision_input_schema_id` to return a GTS schema id; when set, AM validates `root_tenant_metadata` (and the analogous `provisioning_metadata` field on child-create requests) against that schema before invoking `provision_tenant`. The advertised schema id MUST itself be pre-registered in GTS — otherwise bootstrap fails closed with `internal` since the gap is a deploy-time prerequisite, not a payload problem. Plugins inheriting the default `None` keep the opaque pass-through behaviour.) |
