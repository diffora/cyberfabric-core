...
| `cpt-cf-account-management-nfr-data-quality` | Transactional consistency across `tenants` and `tenant_closure`; hierarchy integrity checks | Transactional DB writes spanning `tenants` and `tenant_closure` + SQL-side integrity audit | AM commits hierarchy changes and the corresponding `tenant_closure` updates in a single transaction, so a committed write is observable as one consistent `(tenants, tenant_closure)` state across every reader. Schema stability per `cpt-cf-account-management-nfr-compatibility` ensures the database-level data contract remains intact. AM provides a hierarchy integrity audit (8 SQL classification queries covering missing parents, orphan closure rows, self-edge depth mismatch, missing self-edge, `parent_id` cycles, closure depth mismatch, missing transitive edges, and extra closure edges; see §3.2 *Diagnostic Capabilities*) as a diagnostic capability. | Integration: SQL-side audit detects seeded anomalies (missing parent, missing self-edge, missing transitive edge); Integration: committed writes are immediately visible across both tables via direct query |
| `cpt-cf-account-management-nfr-data-integrity-diagnostics` | Diagnostic checks for observable hierarchy anomalies | `TenantService::check_hierarchy_integrity(scope)` (SQL-side classifier with per-scope single-flight) + observability surface | AM exposes explicit integrity diagnostics via 8 indexed SQL classification queries that run inside a single `REPEATABLE READ ReadOnly` transaction; per-scope single-flight (Postgres advisory lock / SQLite `running_audits` row) bounds concurrent audits and surfaces contention as HTTP `429 Too Many Requests` (`AmError::AuditAlreadyRunning { scope }`). | Integration tests seed each of the 8 anomaly categories and verify the corresponding `Vec<Violation>` output plus per-category metric surfacing; a separate test exercises the single-flight `429` path |
...
**Hierarchy integrity check mechanism**: Exposed as an internal SDK method `TenantService::check_hierarchy_integrity(scope)`. Implementation is SQL-side: 8 indexed classification queries run inside a single `REPEATABLE READ ReadOnly` transaction over `tenants` ⋈ `tenant_closure` (Postgres uses recursive CTEs for the parent-walk shapes; SQLite uses TEXT path concat `path || ',' || id` with `INSTR` for cycle detection, portable across SQLite >= 3.8.3). Each query returns a `Vec<Violation>` directly — there is no in-memory whole-tree load and no in-memory bucketing step — so the audit's memory footprint is `O(violations)`, bounded by the violation count rather than by tenant or closure cardinality. The repository contract is `TenantRepo::audit_integrity_for_scope(scope) -> Result<IntegrityReport>`; the trait surface intentionally hides the underlying SQL so the Postgres and SQLite back-ends can specialize the cycle-detection shape per dialect. The closure-shape categories below are what enforce the **Closure self-row invariant** and **Closure coverage invariant** recorded in the `TenantService` invariants table earlier in this DESIGN; provisioning exclusion (`tenant_closure` never contains rows for tenants in `provisioning` status) is enforced per [AM ADR-0007](ADR/0007-cpt-cf-account-management-adr-provisioning-excluded-from-closure.md) and the DB-level guard `CHECK (descendant_status IN (1, 2, 3))` in [migration.sql](migration.sql), so stale/provisioning rows in `tenant_closure` are a check target rather than a normal state.

*SQL-side classifier — 8 categories* (each is one indexed query; results assembled into a structured report):

- **Missing parent** (`missing_parent`) — `tenant LEFT JOIN tenant ON parent_id WHERE parent IS NULL`; surfaces `parent_id` references that point at a non-existent tenant row, including dangling references created by a concurrent hard-delete. Offending fields: `tenant_id`, `parent_id`.
- **Orphan closure rows** (`orphan_closure_rows`) — closure rows whose `ancestor_id` or `descendant_id` is absent from `tenants` (or whose descendant is in `provisioning` status, which is excluded from closure by construction). Offending fields: `ancestor_id`, `descendant_id`, `closure_descendant_status`, `tenants_status` (or `"missing"`).
- **Self-edge depth mismatch** (`self_edge_depth_mismatch`) — `closure WHERE ancestor_id = descendant_id AND depth <> 0`; the self-row must carry depth 0. Offending fields: `tenant_id`, `observed_depth`.
- **Missing self-edge** (`missing_self_edge`) — `tenant LEFT JOIN closure ON self-edge WHERE closure NULL`; an SDK-visible tenant that has no `(id, id)` row in `tenant_closure`. Offending fields: `tenant_id`, `tenants_status`.
- **Cycle (`parent_id`)** (`cycle`) — recursive CTE on Postgres / TEXT path concat with `INSTR` on SQLite; surfaces tenants reachable from themselves via the `parent_id` chain. Offending fields: `tenant_id`, `cycle_path[]`.
- **Closure depth mismatch** (`closure_depth_mismatch`) — closure rows whose stored depth disagrees with the depth derived by walking `parent_id` from the descendant up to the ancestor. Offending fields: `ancestor_id`, `descendant_id`, `stored_depth`, `expected_depth`.
- **Missing transitive edge** (`missing_transitive_edge`) — `(ancestor, descendant)` pairs present in the `parent_id` walk but absent from `tenant_closure` (parent-walk EXCEPT closure). Offending fields: `descendant_tenant_id`, `missing_ancestor_tenant_ids[]`.
- **Extra closure edge** (`extra_closure_edge`) — closure rows whose `(ancestor, descendant)` pair is not produced by the `parent_id` walk (closure EXCEPT parent-walk). Offending fields: `ancestor_id`, `descendant_id`.

Results are returned as structured diagnostic output — per-category `Vec<Violation>` arrays carrying the offending-row fields listed above — and aggregated into the `am_hierarchy_integrity_violations` gauge metric with a `category` label drawn from the 8 identifiers above (`missing_parent`, `orphan_closure_rows`, `self_edge_depth_mismatch`, `missing_self_edge`, `cycle`, `closure_depth_mismatch`, `missing_transitive_edge`, `extra_closure_edge`) so each indexed query is alerted and dashboarded distinctly. Zero-value emissions occur on clean runs so alert rules see a known baseline.

**Single-flight per scope**: AM enforces at-most-one concurrent audit per scope (whole-tree sentinel or subtree anchor `tenant_id`) so a long-running whole-tree audit cannot pile up against itself. On Postgres the audit takes `pg_try_advisory_lock(hash(scope))` at the start of the snapshot transaction; the lock auto-releases when the transaction ends. On SQLite a `running_audits(scope, started_at)` row is inserted as the lock acquisition and removed in the same transaction. Contention surfaces as `AmError::AuditAlreadyRunning { scope: String }`, which the REST and SDK error-mapping layers translate to HTTP `429 Too Many Requests` per `errors-observability`; callers retry with backoff, AM does not queue.

**Test strategy**: Postgres coverage uses a testcontainers Postgres image (workspace pattern via `cf-modkit-db` dev-dependencies) with a positive and a negative case for each of the 8 categories plus a single-flight contention test asserting the `429` path; SQLite coverage uses `:memory:` databases (portable across SQLite >= 3.8.3) and exercises the TEXT-path-concat cycle detector against both true cycles and deep linear chains to guard against false positives.
...
1. Fetch the schema by `schema_id` from the GTS registry → `404 not_found` with code `metadata_schema_not_registered` if unregistered. Validate the schema body/traits and derive `schema_uuid` deterministically from the same `schema_id`.
...
4. On reads of a specific entry, select the tenant's row by `(tenant_id, schema_uuid)`; if none exists (yet the schema is registered), return `404 not_found` with code `metadata_entry_not_found` so clients distinguish unregistered schemas from unset entries.
...
sequenceDiagram
    ...
    alt Initiator withdraws
        ...
        CS-->>API: 409 conflict {code=invalid_actor_for_transition, attempted_status, caller_side}
        API-->>I: 409
    end
```
...
All errors follow the platform RFC 9457 Problem Details format. The authoritative public `code` values are the exact identifiers enumerated in the OpenAPI `Problem` schema. `code` is required and non-null and is the sole module-specific discriminator clients switch on; the broad RFC 9457 category is conveyed by `status` and does not require a separate envelope field.

| `code` | HTTP Status | Description |
|--------|-------------|-------------|
| `validation` | 422 | Invalid input, missing required fields, or schema validation failure |
| `invalid_tenant_type` | 422 | Tenant type not registered in GTS registry |
| `root_tenant_cannot_delete` | 422 | Attempt to delete the root tenant |
| `root_tenant_cannot_convert` | 422 | Attempt to initiate a mode conversion on a root tenant |
| `not_found` | 404 | Tenant, group, user, or metadata schema/entry not found; stable codes distinguish metadata-schema and metadata-entry cases |
| `metadata_schema_not_registered` | 404 | Metadata `schema_id` is not registered in the GTS types registry |
| `metadata_entry_not_found` | 404 | Metadata schema is registered but the tenant has no direct `(tenant_id, schema_uuid)` entry |
| `conflict` | 409 | Generic conflict that does not have a finer discriminator |
| `type_not_allowed` | 409 | Child tenant type is not allowed under the selected parent type |
| `tenant_depth_exceeded` | 409 | Hierarchy depth exceeds the configured hard limit (strict mode) |
| `tenant_has_children` | 409 | Cannot delete tenant with non-deleted child tenants |
| `tenant_has_resources` | 409 | Cannot delete tenant with remaining resource ownership links in the Resource Group ownership graph |
| `pending_exists` | 409 | A pending conversion request already exists for this tenant; response body carries the existing `request_id` |
| `invalid_actor_for_transition` | 409 | Caller's side is not authorized to drive the attempted `status` transition on the request; response body carries `attempted_status` and `caller_side` |
| `already_resolved` | 409 | Conversion request is no longer pending (any non-`pending` status) |
| `serialization_conflict` | 409 | SERIALIZABLE-isolation retry budget exhausted on a hierarchy-mutating transaction; the losing concurrent writer receives a deterministic conflict envelope per `feature-tenant-hierarchy-management §6 / AC line 711`. Always safe to retry from the client. |
| `cross_tenant_denied` | 403 | Barrier violation or unauthorized cross-tenant access |
| `idp_unavailable` | 503 | IdP contract call failed or timed out |
| `service_unavailable` | 503 | Service is temporarily unavailable |
| `idp_unsupported_operation` | 501 | IdP implementation does not support the requested operation |
...
- integrity diagnostics cover anomalies AM can observe directly via the 8 SQL-side classification queries detailed in §3.2 *Diagnostic Capabilities* — missing parent, orphan closure rows, self-edge depth mismatch, missing self-edge, `parent_id` cycle, closure depth mismatch, missing transitive edge, and extra closure edge — each surfacing as a `Vec<Violation>` from one indexed query inside a `REPEATABLE READ ReadOnly` snapshot transaction
