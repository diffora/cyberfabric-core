...
| `serialization_conflict` | 409 | `conflict` | SERIALIZABLE-isolation retry budget exhausted on a hierarchy-mutating transaction; the losing concurrent writer receives a deterministic `conflict` envelope per `feature-tenant-hierarchy-management §6 / AC line 711`. Always safe to retry from the client. |
