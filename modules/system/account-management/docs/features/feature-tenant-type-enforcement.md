...
> **Staging note.** ACs that assert *positive* type-compatibility rejection (`invalid_tenant_type`, `type_not_allowed`) currently land in two operational regimes:
>
> - **`strict_barriers = true`** (operator opt-in): the barrier fails closed via `strict_barriers_no_uuid_lookup_error()` so any pairing is rejected — useful for environments that must not admit unchecked pairings while the upstream SDK gap (§7 deliberate omission) is open.
> - **`strict_barriers = false`** (default): the barrier reaches the GTS reachability probe and admits every pairing once the registry is reachable (per §7 "wired-but-inert"). In this regime the rejection ACs below describe *intended* behaviour; they exercise the calling saga's mapping path and validate that the hand-off is correct, but a production deployment will not actually surface `invalid_tenant_type` / `type_not_allowed` until the `types-registry-sdk` exposes a UUID-keyed lookup and `GtsTenantTypeChecker::check_parent_child` swaps to the real two-lookup compare. The ACs flip from *intended* to *enforced* without further FEATURE changes once that SDK extension lands.
