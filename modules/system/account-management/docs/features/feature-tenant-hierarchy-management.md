...
- [8. Hierarchy Integrity Audit (SQL)](#8-hierarchy-integrity-audit-sql)
...
- Strict-mode depth exceedance → `conflict` with `code=tenant_depth_exceeded`.
...
- Root tenant deletion → `validation` with `code=root_tenant_cannot_delete`.
- Remaining non-deleted children → `conflict` with `code=tenant_has_children`.
- Remaining Resource-Group-owned resources under the tenant's scope → `conflict` with `code=tenant_has_resources`.
...
3. [ ] - `p1` - **IF** any precondition fails - `inst-flow-sdel-fail-branch`
   1. [ ] - `p1` - **RETURN** the mapped error per `errors-observability` (`validation` / `conflict` with the appropriate `code`) - `inst-flow-sdel-return-error`
...
4. [ ] - `p1` - **IF** type enforcement rejects - `inst-algo-saga-type-reject`
   1. [ ] - `p1` - **RETURN** the mapped error (HTTP `422` with `code=invalid_tenant_type` when type not registered; HTTP `409` with `code=type_not_allowed` when parent type not in `allowed_parent_types` per DESIGN §3.8) - `inst-algo-saga-type-reject-return`
...
6. [ ] - `p1` - **IF** depth evaluation returned strict-reject - `inst-algo-saga-depth-reject`
   1. [ ] - `p1` - **RETURN** `conflict` with `code=tenant_depth_exceeded` - `inst-algo-saga-depth-return`
...
1. [ ] - `p1` - **IF** target tenant is the root (`parent_id IS NULL`) - `inst-algo-sdelpc-root`
   1. [ ] - `p1` - **RETURN** fail with HTTP `422` / `code=root_tenant_cannot_delete` - `inst-algo-sdelpc-root-return`
2. [ ] - `p1` - **IF** target tenant has any non-deleted child — query `dbtable-tenants` for a non-deleted child of `{tenant_id}` (`parent_id={tenant_id}` and `status≠deleted`; `LIMIT 1` existence check) - `inst-algo-sdelpc-children`
   1. [ ] - `p1` - **RETURN** fail with HTTP `409` / `code=tenant_has_children` - `inst-algo-sdelpc-children-return`
...
4. [ ] - `p1` - **IF** any resource association remains - `inst-algo-sdelpc-resources`
   1. [ ] - `p1` - **RETURN** fail with HTTP `409` / `code=tenant_has_resources` - `inst-algo-sdelpc-resources-return`
...
**Output**: Structured diagnostic report assembled from the 8 indexed SQL classification queries enumerated in [Hierarchy Integrity Audit (SQL)](#hierarchy-integrity-audit-sql) and DESIGN §3.2 Diagnostic Capabilities; per-category metric update on `am_hierarchy_integrity_violations` gauge.

**Steps**:

> Categories per DESIGN §3.2 (SQL-side classifier): `missing_parent`, `orphan_closure_rows`, `self_edge_depth_mismatch`, `missing_self_edge`, `cycle`, `closure_depth_mismatch`, `missing_transitive_edge`, `extra_closure_edge`. Each category is one indexed query that returns a `Vec<Violation>` directly from the database — there is no in-memory bucketing, no whole-tree load, and no in-memory size cap.

1. [ ] - `p2` - Acquire per-scope single-flight: on Postgres take `pg_try_advisory_lock(hash(scope))`; on SQLite insert `(scope, started_at)` into `running_audits`. On contention, surface `AmError::AuditAlreadyRunning { scope }` mapped to HTTP `429 Too Many Requests` per `errors-observability` and **RETURN** without running any query - `inst-algo-integ-single-flight`
2. [ ] - `p2` - Open a single `REPEATABLE READ ReadOnly` transaction so all 8 queries observe one consistent `(tenants, tenant_closure)` snapshot - `inst-algo-integ-snapshot-tx`
3. [ ] - `p2` - Inside the snapshot transaction, run each of the 8 indexed classification queries against `dbtable-tenants` ⋈ `dbtable-tenant-closure` (Postgres uses a recursive CTE for the cycle / parent-walk shapes; SQLite uses TEXT path concat with `INSTR` for cycle detection per portable SQLite >= 3.8.3 semantics). Each query returns a `Vec<Violation>` carrying the offending-row fields documented in DESIGN §3.2 - `inst-algo-integ-sql-classifier`
4. [ ] - `p2` - Update the `am_hierarchy_integrity_violations` gauge per `errors-observability` `algo-metric-emission` with a `category` label for each of the 8 categories (zero-value where no anomaly detected so alert rules see a known baseline) - `inst-algo-integ-metric`
5. [ ] - `p2` - Commit the snapshot transaction (Postgres advisory lock auto-releases at txn end; SQLite `running_audits` row deleted in the same transaction) and **RETURN** the structured report - `inst-algo-integ-return`
...
The DELETE flow **MUST** reject root-tenant deletion with HTTP `422` / `code=root_tenant_cannot_delete`, reject deletion when any non-deleted child exists with HTTP `409` / `code=tenant_has_children`, and reject deletion when Resource-Group-owned resources remain under the tenant's ownership scope with HTTP `409` / `code=tenant_has_resources`. On precondition pass, the tenant transitions to `status=deleted` transactionally with the closure `descendant_status` rewrite, and the hard-delete scheduler is informed.

**Current implementation status**: the `root_tenant_cannot_delete` and `tenant_has_children` rejections are wired and tested in `TenantService::soft_delete`. The `tenant_has_resources` rejection is wired through `ResourceOwnershipChecker` with two bindings:

- `RgResourceOwnershipChecker` (production) issues `list_groups($top=1, $filter=hierarchy/tenant_id eq <tenant_id>)` and rejects soft-delete iff any RG row matches. This depends on `cyberfabric-core#1626` having added `hierarchy/tenant_id` to RG's `GroupFilterField` whitelist; until that lands, the OData filter is rejected at validation time on the RG side and the probe surfaces as `code=service_unavailable` instead of `code=tenant_has_resources` — fail-something rather than fail-open, but the precise discriminator only becomes accurate once `#1626` ships.
- `InertResourceOwnershipChecker` (dev / test-only fallback, used when AM boots without a wired RG client) returns `0` unconditionally and admits soft-delete. Production deployments **MUST** bind the Rg variant via `ClientHub`; the inert variant is documented as a stub for environments where RG is intentionally absent.

This DoD flips to `[x]` once `cyberfabric-core#1626` lands in RG so the production probe returns the canonical `tenant_has_resources` rejection on a non-empty tenant.
...
`TenantService::check_hierarchy_integrity()` **MUST** be exposed as an internal SDK method producing a structured report assembled from 8 indexed SQL classification queries defined in DESIGN §3.2 Diagnostic Capabilities and detailed in [Hierarchy Integrity Audit (SQL)](#hierarchy-integrity-audit-sql):

- `missing_parent` — tenant rows whose `parent_id` references a non-existent tenant (LEFT JOIN, parent IS NULL).
- `orphan_closure_rows` — closure rows whose `ancestor_id` or `descendant_id` is absent from `tenants`.
- `self_edge_depth_mismatch` — closure rows where `ancestor_id = descendant_id` but the materialized depth column is non-zero.
- `missing_self_edge` — SDK-visible tenants with no `(id, id)` self-row in closure.
- `cycle` — `parent_id` cycles, detected via recursive CTE on Postgres and TEXT path concat (`path || ',' || id`) with `INSTR` on SQLite.
- `closure_depth_mismatch` — closure rows whose stored depth disagrees with the `parent_id`-walk-derived depth.
- `missing_transitive_edge` — `(ancestor, descendant)` pairs present in the parent-walk but absent from `tenant_closure` (parent-walk EXCEPT closure).
- `extra_closure_edge` — closure rows with no corresponding pair in the parent-walk (closure EXCEPT parent-walk).

The 8 classification queries **MUST** run inside a single `REPEATABLE READ ReadOnly` transaction so the report reflects one consistent `(tenants, tenant_closure)` snapshot, and per-scope single-flight **MUST** be enforced (Postgres advisory lock; SQLite `running_audits` row) so concurrent audits over the same scope surface as `AmError::AuditAlreadyRunning { scope }` mapped to HTTP `429 Too Many Requests`. Memory footprint **MUST** be `O(violations)` rather than `O(tenants + closure)` — no in-memory tenant cap is needed.
...
- [ ] Creating a child at depth > threshold in strict mode returns `409 Conflict` with `code=tenant_depth_exceeded`; the `tenants` table is unchanged.
...
- [ ] DELETE on the root tenant returns `422` with `code=root_tenant_cannot_delete`; DELETE on a tenant with a non-deleted child returns `409` with `code=tenant_has_children`; DELETE on a tenant with remaining Resource-Group-owned resources returns `409` with `code=tenant_has_resources`; DELETE on a childless, resource-free non-root tenant transitions `tenants.status=deleted` and rewrites `tenant_closure.descendant_status=deleted` atomically.
...
- [ ] `TenantService::check_hierarchy_integrity()` returns a structured report with non-empty per-category arrays on a deliberately seeded dataset covering each of the 8 SQL-side anomaly categories, and clean (`[]`) arrays on a known-good hierarchy; the report is produced by 8 indexed SQL classification queries inside a single `REPEATABLE READ ReadOnly` transaction; the `am_hierarchy_integrity_violations` gauge carries the `category` label for every category (zero-valued on a clean run).
...
- **Barrier filtering and `BarrierMode` reductions on read-time queries** — *Owned by `tenant-resolver-plugin`* (DECOMPOSITION §2.9). Hierarchical-action authorization is wired in this feature: every `TenantService` CRUD method calls `PolicyEnforcer::access_scope_with` on `gts.x.core.am.tenant.v1~` with the action vocabulary from DESIGN §4.2 line 1363 (`create`, `read`, `update`, `delete`, `list_children`) before any structural precondition. The compiled `AccessScope` is plumbed into the SQL filter on every disclosure read and mutating write (`find_by_id` of the target tenant, `list_children` over the children collection, `update_tenant_mutable`, `schedule_deletion`) so a PDP-narrowed permit (e.g. `id IN (subset)`) clamps the row set at the database — a constrained authorization cannot widen into a broader closure-based reach. The `tenants` entity is `#[secure(tenant_col = "id", resource_col = "id", ...)]`; the SecureORM compiles the `OWNER_TENANT_ID` / `RESOURCE_ID` constraints onto the `id` column, which is non-widening (an unconstrained PDP permit compiles to `allow_all` and the SQL filter degrades to the legacy `parent_id = …` predicate). Saga-internal structural reads (parent-status precondition, `count_children`, `count_ownership_links`) keep `allow_all` per DESIGN §4.2 line 1370 (structural-read carve-out). `ensure_caller_reaches` (closure-based ancestry) runs after the PEP gate as defense-in-depth — it catches a misconfigured PDP that returns `allow_all` for a non-admin. PDP transport failures fail closed (`service_unavailable` HTTP 503) per DESIGN §4.3.
- **Production tenant-owned-resource counting for the soft-delete `tenant_has_resources` precondition (tenant-keyed `ResourceGroupClient` count)** — *Pending upstream `resource-group-sdk` extension.* The `RgResourceOwnershipChecker` (`modules/system/account-management/account-management/src/infra/rg/checker.rs`) is the production wiring of the `ResourceOwnershipChecker` trait against Resource Group's membership index, and `TenantService::soft_delete` calls it before scheduling retention. The current SDK exposes `list_memberships(query)` filtered by `(group_id, resource_type, resource_id)` only — there is no tenant-keyed projection (the tenant scope lives on the owning `ResourceGroup`, not on the membership row). The production checker therefore ships with a reachability probe that returns `0` once RG is reachable and propagates RG transport failures as `service_unavailable`. When the SDK exposes a tenant-keyed count surface (e.g. `count_resources_owned_by_tenant(tenant_id) -> u64`), only the body of `count_ownership_links` swaps to the real call; the `ResourceOwnershipChecker` trait shape, the `module.rs` ClientHub wiring + `InertResourceOwnershipChecker` fallback, and the test seam stay stable. Until that SDK PR lands, the precondition guard is wired-but-inert in production: the `tenant_has_resources` rejection (acceptance criterion #8) does not fire and a tenant can be soft-deleted while still owning Resource Group memberships.

## 8. Hierarchy Integrity Audit (SQL)

The hierarchy-integrity audit is implemented as a set of indexed SQL classification queries — one per anomaly category — that run inside a single `REPEATABLE READ ReadOnly` transaction on the AM database. The audit produces a per-category report whose memory footprint is bounded by the violation count (not by tenant or closure cardinality), and is gated by a per-scope single-flight so a long-running whole-tree audit cannot pile up against itself.

### Query Catalog

| # | Category                | Query Shape                                                  | Returns         |
|---|-------------------------|--------------------------------------------------------------|-----------------|
| 1 | Missing parent          | tenant LEFT JOIN tenant ON parent_id WHERE parent IS NULL    | Vec<Violation>  |
| 2 | Orphan closure rows     | closure WHERE ancestor/descendant not in tenant              | Vec<Violation>  |
| 3 | Self-edge depth mismatch| closure WHERE ancestor=descendant AND depth<>0               | Vec<Violation>  |
| 4 | Missing self-edge       | tenant LEFT JOIN closure ON self-edge WHERE closure NULL     | Vec<Violation>  |
| 5 | Cycle (parent_id)       | recursive CTE / TEXT path concat (SQLite)                    | Vec<Violation>  |
| 6 | Closure depth mismatch  | closure JOIN parent-walk WHERE expected_depth <> depth       | Vec<Violation>  |
| 7 | Missing transitive edge | parent-walk EXCEPT closure                                   | Vec<Violation>  |
| 8 | Extra closure edge      | closure EXCEPT parent-walk                                   | Vec<Violation>  |

Each query targets one indexed predicate against `dbtable-tenants` and/or `dbtable-tenant-closure`, returns its violations as a `Vec<Violation>` directly, and is composed into the structured report by `TenantService::check_hierarchy_integrity()`. There is no in-memory bucketing step: classification is done by the database via the query shape itself.

### Snapshot Consistency

- A single `REPEATABLE READ ReadOnly` transaction wraps all 8 queries on both Postgres and SQLite, so the report reflects one consistent `(tenants, tenant_closure)` snapshot — concurrent writes do not interleave categories.
- Memory footprint is `O(violations)`, not `O(tenants + closure)`. No in-memory cap on tenant or closure size is required, and no whole-table preload is performed by AM.

### Single-Flight Per Scope

- **Postgres**: `pg_try_advisory_lock(hash(scope))` is taken at audit start; the lock auto-releases when the snapshot transaction ends. Scope is either the whole-tree sentinel or a subtree anchor `tenant_id`.
- **SQLite**: a `running_audits` table holds `(scope, started_at)`; insertion is the lock acquisition and the row is removed in the same snapshot transaction.
- **Conflict surface**: when the lock cannot be taken, `TenantService` surfaces `AmError::AuditAlreadyRunning { scope: String }`, which the REST layer maps to HTTP `429 Too Many Requests` per `errors-observability`. Callers retry with backoff; AM does not queue.

### Test Strategy

- **Postgres**: testcontainers Postgres image — wired through the existing workspace pattern (`cf-modkit-db` dev-dependencies) — runs each of the 8 categories with a positive (violation present) and a negative (clean tree) case, plus a single-flight contention case asserting `429`.
- **SQLite**: `:memory:` database, portable across SQLite >= 3.8.3. The cycle-detection query uses TEXT path concat (`path || ',' || id`) with `INSTR`-based matching, exercised both for true cycles and for false-positive-resistant deep linear chains.
- **Per-category coverage**: every category has at least one positive and one negative integration test; the negative case asserts an empty `Vec<Violation>` and a zero-valued `am_hierarchy_integrity_violations` gauge sample for that `category` label.

### Removed Surface (post-refactor)

The following symbols and configuration knobs are removed by the SQL-side refactor and **MUST NOT** be referenced by new code or new docs outside this historical note:

- In-memory classifiers `classify_tree_shape_anomalies`, `classify_closure_shape_anomalies`, and the in-memory bucketing helper `bucket_by_category` — replaced by the 8 SQL classification queries above.
- Repository loader seam `load_tree_and_closure_for_scope` — replaced by `TenantRepo::audit_integrity_for_scope` (a trait method whose implementation drives the 8 queries inside the snapshot transaction).
- `integrity_max_tenants` configuration knob — no longer applicable; memory is `O(violations)` regardless of tenant count.
- The `IntegrityScope::Whole` whole-tree memory bound — superseded by per-scope single-flight (the audit no longer materializes the tree in memory, so an unbounded whole-tree run is safe; contention is bounded by single-flight, not by a memory cap).
