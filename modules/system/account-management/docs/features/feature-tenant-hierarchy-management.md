...
1. [ ] - `p2` - Open a single `REPEATABLE READ` write-capable transaction (default access mode, **not** `ReadOnly` — the single-flight gate writes inside this tx) so the lock-acquire INSERT, the snapshot SELECTs, and the lock-release DELETE all observe the same MVCC snapshot - `inst-algo-integ-snapshot-tx`
2. [ ] - `p2` - Acquire per-scope single-flight uniformly on both backends, **inside the tx opened above**: insert a `(scope_key, started_at)` row into `running_audits` via SecureORM `secure_insert`. On PK conflict, surface `AmError::AuditAlreadyRunning { scope }` mapped to HTTP `429 Too Many Requests` per `errors-observability`; rollback the tx (no row landed) and **RETURN** without loading the snapshot - `inst-algo-integ-single-flight`
...
**PR1 scope**: domain validator `TenantUpdate::validate_status_transition` rejects PATCH targets `Deleted` / `Provisioning` and current rows in those states with `AmError::Conflict`. The repo-side `update_tenant_mutable` rewrites `tenant_closure.descendant_status` in the same TX as the `tenants.status` flip. REST PATCH handler arrives in a later PR.
...
**Current implementation status**: storage-floor primitives only — the repo-side `schedule_deletion`, `count_children`, and `find_root` methods ship as part of the `TenantRepo` contract. `TenantService::soft_delete`, the `ResourceOwnershipChecker` port, and its `Rg` / `Inert` bindings land with the domain-service layer in a later PR; the DoD's precondition wiring depends on those.
...
- `depth` — `tenants` rows whose stored `tenants.depth` disagrees with the depth derived from the `parent_id` walk (root depth `0`; each step adds `1`). Covers the self-edge case where a `tenant_closure` row has `ancestor_id = descendant_id` AND the corresponding `tenants.depth <> 0` is internally inconsistent — a non-root tenant must have `tenants.depth > 0`. Note the `tenant_closure` table itself carries **no** `depth` column (see `m0001_initial_schema`); the classifier reads `tenants.depth` exclusively.
...
The 8 classifiers **MUST** run synchronously over a `(tenants, tenant_closure)` snapshot loaded via SecureSelect inside a single `REPEATABLE READ ReadOnly` transaction so the report reflects one consistent state, and per-scope single-flight **MUST** be enforced uniformly across PostgreSQL and SQLite via the `running_audits` PK gate so concurrent audits over the same scope surface as `AmError::AuditAlreadyRunning { scope }` mapped to HTTP `429 Too Many Requests`. Memory footprint **MUST** be `O(scope_subtree + violations)` — bounded by the audited subtree size plus the violation count; the trade-off is explicit avoidance of a raw-SQL escape hatch in the production runtime (no `query_raw_all` consumers in production source).
...
- Metric family: `dependency_health` + `am.hierarchy_integrity_violations` gauge (naming-alignment owned by `errors-observability` `dod-ops-metrics-treatment`)
- DB: `cpt-cf-account-management-dbtable-tenants`, `cpt-cf-account-management-dbtable-tenant-closure`
...
**PR1 scope**: `infra::storage::repo_impl::helpers::with_serializable_retry` wraps every transactional repo write under SQL `SERIALIZABLE` isolation with bounded retry on `40001`; `AmError::SerializationConflict` is the typed surface variant. The unique index on `tenants(id)` plus the partial single-root unique index ship in `m0001_initial_schema`. Service-layer callers and racing-saga end-to-end coverage land in subsequent PRs.
...
- **Production tenant-owned-resource counting for the soft-delete `tenant_has_resources` precondition (tenant-keyed `ResourceGroupClient` filter)** — *Coupled to `cyberfabric-core#1626`.* The `RgResourceOwnershipChecker` (`modules/system/account-management/account-management/src/infra/rg/checker.rs`) is the production wiring of the `ResourceOwnershipChecker` trait against Resource Group's group index, and `TenantService::soft_delete` calls it before scheduling retention. As of this revision the checker issues `list_groups($top=1, $filter=tenant_id eq <tenant_id>)` against `ResourceGroupClient` and rejects soft-delete iff the page contains any rows; see the soft-delete-preconditions DoD in §5 for the canonical description. Until `cyberfabric-core#1626` adds `tenant_id` to RG's `GroupFilterField` whitelist, the OData filter is rejected at validation time on the RG side and the probe surfaces as `code=service_unavailable` rather than the canonical `code=tenant_has_resources` (fail-something rather than fail-open; the precise discriminator only becomes accurate once `#1626` ships). `InertResourceOwnershipChecker` remains as a dev/test fallback that admits every soft-delete; production deployments **MUST** bind the `Rg` variant via `ClientHub`. The §5 DoD flips to `[x]` once `#1626` lands.

## 8. Hierarchy Integrity Audit

The hierarchy-integrity audit is implemented as 8 pure-Rust classifier functions that run over an in-memory `(tenants, tenant_closure)` snapshot loaded via SecureSelect inside a single `REPEATABLE READ ReadOnly` transaction on the AM database. The audit produces a per-category report and is gated by a per-scope single-flight via the `running_audits` PK gate (uniform on both PostgreSQL and SQLite), so a long-running whole-tree audit cannot pile up against itself.
...
- **Conflict surface**: when the PK insert fails on conflict, `TenantService` surfaces `AmError::AuditAlreadyRunning { scope: String }`, which the REST layer maps to HTTP `429 Too Many Requests` per `errors-observability`. Callers retry with backoff; AM does not queue.
...
- `integrity_max_tenants` configuration knob — no longer applicable; the memory bound is now `O(scope_subtree + violations)` set by the snapshot-loader rather than a hard pre-flight cap.
- The hard pre-flight whole-tree size guard for `IntegrityScope::Whole` — superseded by the in-memory snapshot model. The audit still materializes the in-scope `(tenants, tenant_closure)` rows for the requested scope (whole-tree or subtree) in process memory at `O(scope_subtree + violations)`, but contention against itself is now bounded by the per-scope `running_audits` single-flight gate (HTTP `429` on conflict) instead of a static row-count threshold.
