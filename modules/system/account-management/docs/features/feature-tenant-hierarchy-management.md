...
**Current implementation status (partial)**: the `root_tenant_cannot_delete` and `tenant_has_children` rejections are wired and tested in `TenantService::soft_delete`. The `tenant_has_resources` rejection is **not yet enforced in production**: the `ResourceOwnershipChecker` trait is wired into the soft-delete path, but both implementations (`RgResourceOwnershipChecker` and the `InertResourceOwnershipChecker` fallback) return `0` unconditionally because the `resource_group_sdk` does not yet expose a tenant-scoped membership-count primitive (see the SDK-gap note in `infra/rg/checker.rs`). The `RgResourceOwnershipChecker` issues a bounded `list_memberships` reachability probe so RG transport failures still surface as `ServiceUnavailable`, but a soft-delete on a tenant that still owns resources will currently succeed. This DoD will flip to `[x]` once the SDK exposes the count primitive and the checker switches from stub-admit to real-count.
