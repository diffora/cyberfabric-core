...
> This algorithm enumerates the 9 public categories — the 8 from PRD §5.8 plus the `too_many_requests` extension introduced for single-flight contention (`AmError::AuditAlreadyRunning` / hierarchy-integrity audit gate, see DESIGN §3.2). Any domain error not matched by steps 2–9 **MUST** fall through to `internal` via step 10 to preserve public-contract stability; the unmatched diagnostic detail is preserved in the audit trail, not the public Problem body.
...
9. [ ] - `p1` - **IF** kind maps to `too_many_requests` (single-flight contention; `AmError::AuditAlreadyRunning { scope }` from the per-scope hierarchy-integrity audit gate per DESIGN §3.2 — at most one concurrent audit per scope, concurrent callers receive a retry-after refusal) - `inst-algo-etp-tmr`
   1. [ ] - `p1` - **RETURN** Problem with HTTP 429 and `code=audit_already_running` - `inst-algo-etp-tmr-return`
10. [ ] - `p1` - **ELSE** unclassified domain error (fallthrough to preserve contract stability) - `inst-algo-etp-fallthrough`
...
**PR1 scope**: `AuditActor` / `AuditEvent` / `AuditEventKind` shapes ship in `cf-account-management-sdk` with `serde` `camelCase` wire format. The `AuditEmitter` runtime, sinks, and the per-call-site `emit_audit` invocations land with the audit-classifier set in a later PR.
