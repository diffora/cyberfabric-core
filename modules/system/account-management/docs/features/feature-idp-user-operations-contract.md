...
- Authenticated tenant-admin GETs `/tenants/{tenant_id}/users` with optional `user_id` filter and pagination params; UserService invokes `IdpPluginClient::list_users` with the tenant filter and projects the response through `gts.cf.core.am.user.v1~`; returns 200 OK with the paginated user list.
