...
**Output**: Pass-through of the plugin-collapsed `Result<(), UserOperationFailure>` — the plugin owns the absent-vs-removed fold, so AM observes a uniform `Ok(())` for both genuine removal and idempotent-absent calls.
