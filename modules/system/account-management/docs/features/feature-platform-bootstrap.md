...
8. [ ] - `p1` - **IF** classification = `invariant-violation` (suspended or deleted status on root row — illegal pre-existing state) - `inst-flow-bootstrap-branch-invariant`
   ...
   3. [ ] - `p1` - **RETURN** `invariant_violation` error (root in illegal state — manual intervention required) - `inst-flow-bootstrap-return-invariant`
...
- [ ] Concurrent replica starts on a fresh database: exactly one replica creates the root; other replicas hit the `ux_tenants_single_root` unique constraint on their insert attempt and fall through to the idempotency path, returning `bootstrap.skipped`; no duplicate `tenants` or `tenant_closure` rows exist.
